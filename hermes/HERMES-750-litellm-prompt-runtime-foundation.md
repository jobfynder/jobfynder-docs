...
**No live verification performed for this update** — based on the commit diff and its accompanying tests (`tests/prompt_runtime/test_local_resume_extract.py`, verified passing this run: `pytest tests/prompt_runtime/` → 6 passed) only; no SSH/API check was run against INTEL-1 to confirm the deployed registry_version live.
