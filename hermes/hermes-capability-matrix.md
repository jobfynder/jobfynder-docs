...
| HERMES-750 | Prompt Runtime Foundation | ✅ | Closed foundation (tag `hermes-750-prompt-runtime-v1`), rewritten for LiteLLM 2026-08-21 — see `hermes/HERMES-750-litellm-prompt-runtime-foundation.md`. Dry-run-first, RBAC-protected (`agents:run`), safety-checked before every call. A wrong Langfuse API key and a real N+1 performance issue (33s cold-cache prompt fetch) were found and the key issue fixed during that rewrite — see the doc for both |
