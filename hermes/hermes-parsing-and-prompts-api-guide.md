...
**Superseded the next day, then fixed the day after that.** Re-checked live 2026-09-08 (morning): `registry_version` was `hermes_prompt_registry_v1` (reused from the original 2026-07-10 static registry — see `hermes/HERMES-750-litellm-prompt-runtime-foundation.md` §11) and `prompt_count` was **42**, not 38. Cause: commit `8319c43` on `jobfynder/hermes` (2026-09-07, primarily a resume-parsing improvement — see `hermes/HERMES-200-understanding-foundation.md`) added a local fallback prompt registry for resilience against a Langfuse outage, but merged it into `list_prompts()` as a **union**, not a fallback used only when Langfuse is unreachable — so the 4 old, already-retired prompt IDs it contains resurfaced in the live registry unconditionally.

**Fixed 2026-09-08 (afternoon), commit `a745fed` on `jobfynder/hermes` `main`.** The 4 dead legacy prompt IDs were removed outright from `app/prompt_runtime/registry.json` (not just hidden behind different merge logic). Code-verified this run: `registry.json` now holds exactly 2 entries (`jf.onboarding.profile-import.extract`, `jf.resume.parse`), and `list_prompts()` with Langfuse unconfigured returns `prompt_count: 2`, down from the pre-fix 6. Since both survivors share their `prompt_id` with an existing Langfuse prompt, the live `GET /prompts/registry` `prompt_count` is expected to read **38** again, and `registry_version` to read `hermes_prompt_registry_v1` (now legitimately meaning "merged," per HERMES-750 §11 — not the old static registry). **Not live-verified** — no SSH/API credentials configured in this environment to re-pull the endpoint against INTEL-1 and confirm 38 directly. **The §3a table below is now historical** — those 4 prompt IDs no longer exist in `registry.json` as of `a745fed` and should not currently be returned by the live endpoint. Full trail: `hermes/HERMES-750-litellm-prompt-runtime-foundation.md` §11 and PR #19 on `jobfynder/jobfynder-docs`.
...
Every prompt here is fetched live by Hermes from Langfuse (project `jobfynder-ai`) — this file is a point-in-time snapshot for discoverability, not the runtime source. Since 2026-09-07, two of the fallback-extraction prompts below (`jf.resume.parse`, `jf.onboarding.profile-import.extract`) also have a hardcoded local copy (`app/prompt_runtime/registry.json`) that `GET /prompts/registry` merges in — briefly (2026-09-07/08) that file also carried four other, retired local-only prompt IDs that leaked into the merged listing; those were deleted in commit `a745fed` (2026-09-08) — see the note above and `HERMES-750-litellm-prompt-runtime-foundation.md` §11. For the always-current version, call `GET /prompts/registry` yourself or `GET /prompts/{prompt_id}` for one prompt's full detail (includes the actual `system_template`/`user_template` text, omitted below for brevity — see `hermes-complete-developer-guide.md` §11 for the `/prompts/run` execution contract).
...
That's 30 genuine-generation prompts, bringing the total to **38** (8 fallback + 30 generation) — the count as of 2026-09-07, and expected again as of 2026-09-08 after the fix (code-verified, not live-confirmed). See §3a for the historical record of the 4 that briefly resurfaced in between.

## 3a. Resurrected legacy prompts (historical — fixed 2026-09-08, commit `a745fed`)

**Removed from `registry.json` as of commit `a745fed` on `jobfynder/hermes` `main` (2026-09-08) — kept below only as a historical record of the 2026-09-07/08 regression, not a description of current behavior.** These 4 IDs should no longer appear in `GET /prompts/registry` once `a745fed` is deployed (not live-confirmed — see the status note at the top of this file).

Re-verified live 2026-09-08 (morning, before the fix): these 4 appeared in `GET /prompts/registry` alongside the 38 above, bringing the reported count to 42. Each is the original 2026-07-10 static-registry entry (`default_model` is a literal provider string, `anthropic/claude-haiku-4-5`, not a LiteLLM router alias — structurally different from every prompt in §2/§3, which carry `metadata.execution_class`/`litellm_router_alias`). They were retired from active use when Langfuse replaced the static registry (2026-08-21) but resurfaced as a side effect of commit `8319c43`'s local-fallback-registry merge (see the status note at the top of this file). **Each duplicates an existing `jf.*` prompt — treat the `jf.*` version as authoritative, not these:**
...
**Resolved 2026-09-08, commit `a745fed`:** deletion, not a merge-behavior change — the 4 IDs were removed from `registry.json` outright. `list_prompts()`'s merge logic (`{**local, **langfuse_cache}`) is unchanged; there is simply nothing left in the local file for these 4 IDs to leak from. See `hermes/HERMES-750-litellm-prompt-runtime-foundation.md` §11 for the full trail.
