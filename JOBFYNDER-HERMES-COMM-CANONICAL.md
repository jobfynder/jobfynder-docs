...
**Update 2026-09-08.** "More resume fields" is now partially closed: commit `8319c43f` on `hermes` `main` (2026-09-07, `feat(understanding): extract resume sections and international phones without an LLM`) added a deterministic resume-section parser (`app/understanding/parsers/resume_sections.py`) producing `name`, `location`, `summary`, structured `experience` entries (company/title/dates/location/description), structured `education` entries (institution/degree/field/year), and `certifications`, plus international phone-number extraction (labeled-line pattern + the `phonenumbers` library) in `app/understanding/parsers/contact.py`. See `hermes/HERMES-200-understanding-foundation.md` for the full evidence trail. Bigger taxonomy, more JD fields, real Unstructured.io integration, and Langfuse/Promptfoo/Great Expectations evaluation remain open.
...
**Update 2026-09-08.** Commit `8319c43f` on `hermes` `main` (2026-09-07, same commit as the HERMES-200 update above) added a local, on-disk prompt-registry fallback (`app/prompt_runtime/local_prompts.py` + `app/prompt_runtime/registry.json`, two prompts: `jf.onboarding.profile-import.extract`, `jf.resume.parse`). `list_prompts()`/`get_prompt()` now merge this local registry with whatever is cached from Langfuse instead of returning an empty registry or `None` when Langfuse is unconfigured or a prompt ID isn't found there; `registry_version` for the merged result reverts to `hermes_prompt_registry_v1` (the original 2026-07-10 name). This does not change the dry-run/RBAC production contract. See `hermes/HERMES-750-litellm-prompt-runtime-foundation.md` §11 for the full evidence trail.
