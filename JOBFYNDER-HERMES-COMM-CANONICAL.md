...
**2026-09-11 (v1.11):** Scheduled docs-sync run. Thirteen new commits on `jobfynder/hermes` `main` since v1.10 (`5f239f79..061aee28`), spanning HERMES-850, HERMES-400, and HERMES-100. Highlights: PR #11 (`fix/hermes-850-full-reparse-batch-progress`), flagged in v1.10 as still open, has merged (as `e4071bfa`) — the full-reparse backlog now actually advances past unchanged rows. Two real production incidents were found and fixed: a synchronous NER-model cold-load on the signature-parsing fallback path that was indistinguishable from a request hang, and a RabbitMQ-consumer starvation risk on the Microsoft Graph notification path under CPU load. The old synchronous backfill endpoints are retired in favor of a new durable, pausable, resumable backfill job queue (two new Postgres tables, a dedicated worker process, four new endpoints); a companion `review-reparse` backfill kind and a new server-paginated review dashboard (replacing full-list client-side filtering) also landed, along with a conservative rule that clears a stale `needs_review` flag back to `draft` only when the parser's own recorded output was already clean — never to `published`. Several deterministic email-title-recovery bugs were fixed. On the taxonomy side (HERMES-400), a new noise/non-technical-skill filter was added, daily auto-triage's LLM auto-approval path is now fully disabled for every candidate type (skill/job_title/boilerplate_line), and two LLM-backed taxonomy helpers are now opt-in rather than default-on. A new Postgres advisory-lock reader/writer pattern in `app/runtime/db.py` (HERMES-100) now serializes schema migrations against concurrent intake/backfill/API-vs-worker writes. See the HERMES-850, HERMES-400, and HERMES-100 sections below for the full evidence trail. No new commits on `jobfynder-infra` `main` — its HEAD is unchanged at `2622899`, already recorded as the last commit in this document. Live verification skipped: no SSH credentials configured in this environment. **Amended same day:** a reviewer on the PR carrying this update flagged that `jobfynder/hermes` `main`'s CI has actually been red for the last 5 pushes (`73bb5d7b..061aee28`) — independently re-verified against the GitHub Actions API and the failing job's own log, confirmed a real regression (not a flake), and HERMES-850's Testing dimension is downgraded to 🔴 accordingly. See the HERMES-850 section and the new P0 item in [§8](#8-priority-matrix).
...
**Update (2026-09-11, v1.11).** Two commits harden `app/runtime/db.py`'s schema-initialization path against concurrency, motivated by the new multi-process workers introduced this cycle (the backfill worker and the Graph consumer, both under HERMES-850 below). `5c0dceab` (`Serialize schema initialization across API and workers`) has `init_schema()` take a Postgres advisory lock (`pg_advisory_xact_lock`) before running its `CREATE TABLE IF NOT EXISTS` block, since that statement can still race in Postgres's type catalog when multiple processes start against a new schema concurrently; new test `test_concurrent_schema_initialization` runs `init_schema()` from 4 threads at once and confirms no exception. `9cd50459` (`Coordinate schema migrations with active intake and backfill transactions`) extends this to a reader/writer pattern: ordinary intake/backfill transactions now take the same lock in **shared** mode via `transaction()`/`cursor()`, while `init_schema()` takes it in **exclusive** mode — so a schema migration can no longer run concurrently with in-flight intake or backfill writes, or vice versa. New test `test_schema_waits_for_intake_before_taking_table_locks` opens a transaction on one thread and confirms a concurrent `init_schema()` call on another thread blocks until it completes, rather than proceeding alongside it. This does not change HERMES-100's maturity band or dimension grades — it is internal hardening of already-🟢-graded Implementation/Deployment, not new scope.
...
**Update (2026-09-11, v1.11).** Four commits harden this module's existing safety rule ("unknown terms are never auto-approved") and its lookup performance:

- `15c0d260` (`Index pending taxonomy candidate matching to unblock live email intake`) replaces an unindexed full-table scan in `_find_loosely_matching_pending_candidate()` (`app/understanding/taxonomy/candidates.py`) with a single query backed by a new partial expression index, `idx_taxonomy_candidates_pending_loose` (`app/runtime/db.py`). New test `tests/test_candidate_lookup.py` includes `test_index_supports_lookup`, which runs `EXPLAIN` with `enable_seqscan=off` and asserts the new index actually appears in the query plan, not just that results are correct.
- `b63bdd72` adds `app/understanding/taxonomy/skill_quality.py` (new): `skill_noise_reason()`, a conservative, regex-only validator (no model calls) that rejects a fixed set of ~50 observed-bad standalone terms, leading sentence-fragment connectives, unbalanced parentheses, and URL/`@`-containing strings — wired into candidate promotion (`_is_noise_skill_term()`), manual approval (`approve_taxonomy_candidate()`, which now rejects a noise term with `reason: 'invalid_skill_name'`), and `add_canonical_skill()` (raises before any DB write). `fuzzy_match_terms()` (`app/understanding/parsers/skills.py`) no longer fuzzy-matches any skill whose `source == "taxonomy_candidate_approved"` — a learned term can now only match document text exactly, not via partial-ratio fuzzy matching.
- `061aee28` widens `skill_noise_reason()` with four more checks targeting non-technical/soft-skill terms (stakeholder/leadership/teamwork/interpersonal/time-management/negotiation-type language; bare "communications" unless paired with a technical qualifier; "collaboration"/"presentations" unless paired with a named tool or "protocols"/"tools"). New test `test_technical_only_policy` (`tests/test_skill_quality.py`) confirms both the new rejections and that legitimate technical terms sharing vocabulary with rejected phrases (e.g. "Communication Protocols," "Unified Communications," "Microsoft Teams") are not caught by the filter.
- `b63bdd72` and `061aee28` together progressively disable `scripts/hermes-900-daily-taxonomy-triage.py`'s LLM-based auto-approval: first for `signal_type == "skill"` only, then widened to all three candidate types — `skill`, `job_title`, `boilerplate_line`. This daily job no longer calls its LLM classifier or auto-approves any taxonomy candidate at all; only human review or the now noise-filtered manual-approval path can promote one.
- Separately, `061aee28` makes two existing LLM-backed helpers opt-in rather than default-on: `generate_skill_description()` (`app/understanding/taxonomy/descriptions.py`) and `classify_job_title_family()` (`app/understanding/taxonomy/title_family_classifier.py`) both gain an `allow_llm=False`-by-default keyword, so a caller that doesn't explicitly opt in gets only the deterministic path even when LiteLLM is configured and reachable. New test `tests/test_deterministic_taxonomy.py` confirms the mocked LLM call is never invoked for either function absent explicit opt-in.

**Naming note.** `scripts/hermes-900-daily-taxonomy-triage.py` (touched by the two commits above) is a taxonomy-candidate auto-triage script and, based on the code actually touched in this batch, has no connection to the HERMES-900 "Role Intelligence Packages" module — nothing in it resembles role-based capability packages. This reads as a coincidental reuse of the number "900" in a script filename, not a naming collision with the HERMES-900 module (see that section below), but this is inferred from the absence of related code rather than confirmed against an authoritative ticket/module-numbering source — flagged here for a human to confirm, in the same spirit as the COMM-1 naming collision resolved in [§2.1](#21-comm-1-has-been-used-to-mean-two-different-things).

None of these five commits change HERMES-400's maturity band or dimension grades — Architecture, Implementation, and Testing were already 🟢, and this batch is safety/performance hardening within the existing design, not new scope.
...
**Update (2026-09-11, v1.11).** PR [#11](https://github.com/jobfynder/hermes/pulls/11), flagged directly above as open and undocumented, has merged — landed on `main` as commit `e4071bfa` (`fix(drafts): make bounded full-reparse batches actually advance through the backlog`, 2026-09-08T21:41Z). `backfill_full_reparse()`'s candidate query (`app/drafts/service.py`) previously ordered by `created_at` with no offset, so a row whose re-parse reproduced the same confidence (including every genuinely-unresolvable draft) was never written and kept reappearing at the front of every bounded `limit=` call — a batch could never advance past a large contingent of permanently-stuck rows. Fixed by ordering by `updated_at` instead, and (`dry_run=False` only) always bumping a no-op row's `updated_at` via a lightweight `UPDATE...SET updated_at = now()`. New test (`scripts/hermes-850-signature-company-fill-check.py`) confirms a stuck draft's `updated_at` moves while its payload stays byte-identical. Twelve more commits followed, continuing past `5f239f79`:

`2d7c24c` (`perf(drafts): batch full-reparse writes into one transaction instead of one per row`) is a direct follow-up: re-parsing itself measured ~0.5s/draft (CPU-only, per the commit's own account), but each row's write path opened its own database cursor/commit for both the drafts `UPDATE` and `record_field_provenance`'s `INSERT`, and a 100-row apply batch reportedly did not finish in 180s in production — figures from the author's own measurement, not independently verified in this diff. `backfill_full_reparse()` is now split into a CPU-only collection phase and a single-transaction write phase (all UPDATEs, no-op `updated_at` touches, and provenance inserts committed together); `record_field_provenance()` (`app/email_parsing/provenance.py`) gained an optional `cur` parameter to reuse an already-open cursor, with every existing call site unaffected when it isn't passed.

`8917a375` (`fix(startup): load the NER model at app startup, not on the first request that needs it`) is a real, live-debugged production incident: a request landing on `parse_email_signature`'s NER fallback path after a container restart paid the full synchronous cost of loading `en_core_web_sm` — "well over ten minutes" on this deployment's hardware, per the commit message — indistinguishable from a hang, while other endpoints and the database sat idle throughout. Fixed with a new `warm_ner_pipeline()` (`app/email_parsing/signature.py`) called from a FastAPI startup event in `app/main.py`, moving the cost to container boot; confirmed safe when spaCy/the model isn't installed (existing no-raise behavior preserved). New test `test_warm_ner_pipeline_never_raises`.

**New durable backfill job-queue subsystem** (commit `342cd499`, `app/drafts/backfill.py` — new, 177 lines — plus `scripts/hermes-backfill-worker.py`, new endpoints in `app/routers/drafts.py`, and new tables in `app/runtime/db.py`). Replaces the old fire-and-forget synchronous backfill endpoints with a resumable, pausable job model: `create_job()` snapshots eligible drafts into a new `draft_backfill_items` table under one `draft_backfill_jobs` row (an advisory lock, `pg_advisory_xact_lock(892310480)`, enforces only one unfinished job at a time); `control_job()` handles `pause`/`resume`/`cancel`; `run_batch()` re-parses a batch outside any write transaction, then commits all resulting drafts/provenance/outcome writes together, re-checking each draft's `updated_at`/human-correction status under `FOR UPDATE` so a concurrently-edited draft is skipped rather than overwritten. `scripts/hermes-backfill-worker.py` is a new standalone single-instance worker process (`pg_try_advisory_lock(892310481)` enforces single-instance) polling for queued jobs. Four new endpoints: `POST /drafts/backfill/jobs`, `GET /drafts/backfill/jobs`, `GET /drafts/backfill/jobs/{job_id}`, `POST /drafts/backfill/jobs/{job_id}/{pause|resume|cancel}` (`drafts:publish`/`drafts:read`). The **old** synchronous endpoints (`/drafts/backfill/full-reparse`, `/drafts/backfill/signature-company-fill`) are now restricted to dry-run previews only — a real (`dry_run=False`) call now returns HTTP 409 directing the caller to the new job-based endpoint, and `limit` is capped to 20. Commit `73bb5d7b` adds a third backfill kind, `review-reparse`, scoping a job's item snapshot to job-requirement drafts currently in `needs_review` status only. New tests in `tests/test_production_reliability.py` (`BackfillIntegrationTests`) cover pause/resume, idempotent job creation, atomic rollback on a mid-batch failure (zero drafts show a partial change after an injected error), and that a concurrently-edited draft is skipped.

**Microsoft Graph consumer: heartbeat-starvation fix** (commit `342cd499`, `scripts/hermes-850-graph-notification-consumer.py`, `app/providers/microsoft_graph/service.py`, `docker-compose.yml`). The consumer previously ran each notification's CPU-bound processing synchronously inside the same thread handling RabbitMQ's heartbeat, risking the connection being dropped as unresponsive under load. Processing now dispatches to a spawned `ProcessPoolExecutor` (single worker, 120s deadline) while the parent process keeps polling RabbitMQ (`process_data_events(time_limit=0.25)`) and touches a heartbeat file (`/tmp/hermes-graph-heartbeat`) every cycle; `prefetch_count` drops from 10 to 1; Pika's own `heartbeat`/`blocked_connection_timeout` are now explicitly set (60s/120s). `fetch_graph_message()` gains a typed `GraphFetchError`; HTTP 404/410 now dead-letter immediately instead of retrying, and a 429's `Retry-After` header is honored against the existing backoff schedule. `docker-compose.yml`'s `hermes-graph-consumer` service gains a healthcheck against the heartbeat file's mtime (and a corresponding new `hermes-backfill-worker` service/healthcheck for the subsystem above). New integration check `scripts/hermes-graph-heartbeat-check.py` runs a synthetic 9-second CPU-bound job against a disposable RabbitMQ instance with a 2-second heartbeat interval and confirms the message is still acknowledged. A companion operations doc, `docs/graph-production-operations.md`, records a write-inclusive benchmark (5/20/100-row batches at 6.4/6.6/24.2s) — this is prose in that doc, not demonstrated by a test in this diff, and is recorded here as the author's claim, not independently verified.

**New server-paginated review dashboard** (commit `3e578788`, new files `app/drafts/listing.py` and `app/drafts/review_rules.py`) — an addition to the drafts-review workflow, not an extension of the deterministic email parser itself. `list_draft_page()` moves review-dashboard filtering/search/counting/pagination into a single indexed SQL query (previously the frontend fetched the full draft list and filtered client-side); the frontend (`frontend/src/pages/DraftListPage.tsx`) was rewritten to match. New endpoint `GET /drafts/page` (`drafts:read`). Separately, `reconcile_review_status()` (`review_rules.py`) is a conservative, audited rule: it clears a draft's stale `needs_review` flag back to plain `draft` status **only** when the parser's own already-recorded output is fully clean by a strict predicate (`parser_is_ready()` — confidence ≥0.7 at both draft and every record level, no errors/warnings, `requires_review` false throughout) and the draft carries no human reviewer/recruiter correction — it never sets a draft to `published`. New endpoint `POST /drafts/review/reconcile-ready` (`drafts:publish`, `dry_run=True` by default). New tests `tests/test_dashboard_listing.py` and `tests/test_review_rules.py` (pagination correctness/clamping, duplicate exclusion, literal-not-wildcard search, and `parser_is_ready()`'s accept/reject cases). A companion doc, `docs/dashboard-review-operations.md`, cites a production measurement (old endpoint: 15,117 records / ~20.35MB / 2.69s vs. new first page: 50 records / ~27KB / 0.34s) — again prose in that doc, not verified by a test here.

**Email title-recovery and false-split fixes** (commit `73bb5d7b`, `app/understanding/parsers/basic.py`, `app/email_parsing/parsers.py`, `app/email_parsing/signature.py`, `app/understanding/parsers/contact.py`). `extract_probable_title()` now unescapes HTML entities, accepts `urgent`/`immediate` hiring prefixes, no longer rejects titles starting with a dot (fixing `.NET Developer`-style titles), and recognizes more title keywords (designer, researcher, technician, scientist, specialist, tester). A new `_extract_heading_title()` fallback recovers a title from a `Job Title:`/`Title:`/`Position:`/`Role:` heading-and-value pair. `_numbered_position_sections()` and `_split_requirement_sections()` gain guards against misreading a task/responsibilities list, a contact's own `"Role: Technical Recruiter"` signature line, or a screening-steps list as additional job postings. `SIGNOFF_RE` now also matches `"Thanks & regards,"`. Work-authorization extraction gains an `"Any Visa"` pattern. New test file `tests/test_email_review_regressions.py` (14 cases) and companion doc `docs/email-review-repair.md`.

**Relay-signature spacing fix** (commit `b63bdd72`, `app/email_parsing/signature.py`): `RELAY_FROM_BLOCK_RE` now tolerates up to 3 blank lines between the relay block's `From:`/name/company/email lines, where it previously required them adjacent. New test `tests/test_relay_spacing.py` also confirms a `From:` block missing a company value is not incorrectly matched.

**Deterministic-parsing LLM-skip flags** (commit `b5a245c5`): `app/channels/service.py`'s email-channel intake path and `app/email_parsing/parsers.py`'s `parse_requirement_email()` now both pass `skip_llm_fallback=True` into `understand_document()` (a parameter that already existed on that function outside this commit's diff) — reinforcing the deterministic-first policy already recorded for this module. New test `tests/test_email_fallback_boundary.py` also locks in, as a regression test, that a successful LLM fallback (when one does run) still preserves the original raw input text — this documents a protected invariant, not a bug fix; the diff does not show input text ever being lost previously.

None of the new `app/drafts/backfill.py`/`listing.py`/`review_rules.py` subsystems, the new `/drafts/backfill/jobs*`, `/drafts/page`, or `/drafts/review/reconcile-ready` endpoints, or the Graph-consumer/title-recovery/relay-spacing fixes above have been added to `hermes/HERMES-850-email-parsing-foundation.md` or `hermes-api-route-inventory.md`, which remain frozen as of the module's 2026-08-21 closure — this section is the current record for these changes until/unless those files are revised.

**Observability bumps to 🟡 Partial** (from 🔵 Designed) on the strength of this update: the new `hermes-graph-consumer` and `hermes-backfill-worker` Docker services both now carry heartbeat-file-based healthchecks where neither process previously had any liveness signal at all. This remains 🟡, not 🟢 — healthchecks give container-level liveness only, with no metrics/alerting beyond Docker's own health status (see [§8](#8-priority-matrix)).

**CI is currently red on `jobfynder/hermes` `main` — Testing downgraded to 🔴 Gap/Rework.** Caught by a reviewer on the PR that introduced the update above, and independently re-verified against the GitHub Actions API directly (not taking the reviewer's word for it): the "Hermes regression suite" workflow has failed on every one of the last five pushes to `main`, starting exactly at `73bb5d7b` and continuing through the current HEAD, `061aee28` — it has not self-resolved:

- run [69](https://github.com/jobfynder/hermes/actions/runs/34520846850) (`73bb5d7b`) — `conclusion: failure`
- run [70](https://github.com/jobfynder/hermes/actions/runs/34529981733) (`15c0d260`) — `failure`
- run [71](https://github.com/jobfynder/hermes/actions/runs/34625250171) (`b63bdd72`) — `failure`
- run [72](https://github.com/jobfynder/hermes/actions/runs/34625845154) (`b5a245c5`) — `failure`
- run [73](https://github.com/jobfynder/hermes/actions/runs/34626987597) (`061aee28`, current HEAD) — `failure`

The immediately prior run, [68](https://github.com/jobfynder/hermes/actions/runs/34468669269) (`3e578788`), is `success` — so the break lands exactly on `73bb5d7b`, not before. The backend job's log (retrieved and read directly, not inferred from the run's pass/fail status alone) shows the same failure on every one of the five runs, at the same assertion:

```
scripts/hermes-850-email-parsing-check.py, test_position_ordinal_label_is_never_read_as_the_job_title
AssertionError: Must read the real title from 'Title:', never the ordinal from 'Position:', got ['Java Developer with NoSQL and State Client Exp']
```

This is a pre-existing regression test (guards a real prior production bug, predates this commit range) expecting a two-posting email to resolve to two distinct titles. `73bb5d7b`'s new `_extract_heading_title()`/`_split_requirement_sections()` changes (documented above) appear to have broken that multi-posting split, and none of the four commits after it (through `061aee28`) fixed it — this is a real regression sitting on `main` right now, not a flake (same assertion, same commit range, five consecutive runs) and not something this update's own narrative above should be read as having verified clean. Per this document's own rule ("never mark a module more done than the evidence supports"), Testing for HERMES-850 is downgraded from 🟢 to 🔴 until this is fixed and a subsequent push to `main` shows the regression suite green again. This does not retroactively unverify the individual code-level claims made in the update above (each was checked against its own diff/tests independently) — it means the *combined* state of `main` right now does not pass its own test suite, which is a fact this document cannot omit. See the new P0 item in [§8](#8-priority-matrix).
...
**Naming note (2026-09-11).** `scripts/hermes-900-daily-taxonomy-triage.py` exists in the codebase and was touched by recent HERMES-400 work (see the HERMES-400 section above), but it is a taxonomy-candidate auto-triage script, unrelated to Role Intelligence Packages — flagged as a probable coincidental filename numbering, not confirmed against an authoritative module-numbering source. This module's own status (no implementation, no file) is unchanged.
...
| **P0** | Fix the red `jobfynder/hermes` `main` CI (Hermes regression suite) | Failing on the last 5 pushes (`73bb5d7b`..`061aee28`), same assertion every time (`test_position_ordinal_label_is_never_read_as_the_job_title`), root-caused to `73bb5d7b`'s heading/section-split changes. Not a flake. See the HERMES-850 section above. |
...
| **P2** | Add alerting beyond Docker-healthcheck liveness for the new `hermes-backfill-worker`/`hermes-graph-consumer` processes (HERMES-850) | Both gained heartbeat-file-based container healthchecks 2026-09-11 (commit `342cd499`), but nothing pages a human if either stays unhealthy — only Docker's own health status reflects it. |
