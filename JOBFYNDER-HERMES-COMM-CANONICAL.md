...
**Update 2026-09-07 (commit `8319c43` on `jobfynder/hermes`).** The resume parser gained deterministic (no-LLM) section extraction it did not have before: `name`, `location`, `summary`, structured `experience` (company/title/dates/location/description), structured `education` (institution/degree/field/year), and `certifications` — new `app/understanding/parsers/resume_sections.py`, wired into `parse_basic_structured_data()`. `extract_phone()` (`app/understanding/parsers/contact.py`) now also tries the `phonenumbers` library for international numbers before falling back to the old regex. The prompt runtime's Langfuse-fallback path also gained a local prompt registry entry for `jf.resume.parse` (and `jf.onboarding.profile-import.extract`), so the resume LLM-fallback no longer returns nothing when Langfuse is unreachable — `jf.jobs.jd.extract` (the JD side) still has no local counterpart. Evidence is three new unit-test files (`tests/understanding/test_resume_sections.py`, `tests/understanding/test_basic_resume_parse.py`, `tests/prompt_runtime/test_local_resume_extract.py`); **no live-deployment verification was performed for this change** (this environment has no SSH credentials configured for INTEL-1 — see the note in [§13](#13-keeping-this-document-current-automatically)). See `hermes/HERMES-200-understanding-foundation.md` for the full breakdown.

**Gap.** "More resume fields" (the file's own former "Next" item) is now done as of the update above. Still open: bigger taxonomy, more JD fields, real Unstructured.io integration, Langfuse/Promptfoo/Great Expectations evaluation — none confirmed done as of this document.
...
**Note 2026-09-07 (commit `8319c43` on `jobfynder/hermes`).** `app/prompt_runtime/langfuse_prompts.py`'s merged-registry function (`_merged_prompts()`) now falls back to a small locally-defined prompt registry (`app/prompt_runtime/local_prompts.py` reading `registry.json`) when Langfuse doesn't have a given prompt ID, currently covering `jf.resume.parse` and `jf.onboarding.profile-import.extract`. This is additive resilience for two specific prompt IDs, not a change to the 38-prompt Langfuse catalog itself, and does not close any of the five open items above.
...
| **P2** | Add a local Langfuse-fallback prompt for `jf.jobs.jd.extract` | Commit `8319c43` (2026-09-07) added a local fallback for `jf.resume.parse` but not the JD-side equivalent — the JD LLM-fallback path still goes empty if Langfuse is unreachable. |
