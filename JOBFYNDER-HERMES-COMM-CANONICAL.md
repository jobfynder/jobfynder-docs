...
**2026-09-09 (v1.11):** Scheduled docs-sync run. Three new commits on `jobfynder/hermes` `main` since v1.10 (`5f239f79..8917a375`), all HERMES-850: `e4071bfa` is PR #11 landing on `main` — fixes `backfill_full_reparse` so a bounded `limit=` call actually advances through the backlog instead of re-selecting the same oldest rows forever; `2d7c24cd` is a follow-on performance fix found while about to run that batch against production — a 100-row apply didn't finish in 180s because every row opened its own database transaction, so the function was split into a CPU-only re-parse phase and a single-transaction write phase; `8917a375` fixes a real production incident (a cold NER-model load masquerading as a hung request) by warming the model at container startup instead of on the first live request that needs it. See the HERMES-850 section below for the full evidence trail. No new commits on `jobfynder-infra` `main`, and the deployed COMM branch (`feature/comm-telegram-message-chunking`) is unchanged — its tip is still `0c33580`. Live verification skipped: no SSH credentials configured in this environment.
...
A seventh commit, `fix(drafts): make bounded full-reparse batches actually advance through the backlog` (PR [#11](https://github.com/jobfynder/hermes/pulls/11), branch `fix/hermes-850-full-reparse-batch-progress`), was opened against `main` during the v1.10 run and had not yet merged as of that sync. It has since merged as `e4071bfa62b468af1997da5f00434c77ba1a887b` — see the v1.11 update below for its content and two more commits that followed it.

**Update (2026-09-08→09, v1.11).** Three more commits landed on `main`, all continuing the `backfill_full_reparse` work `f1dd1ce42` (v1.10) introduced:

- `e4071bfa` (PR #11, merged) fixes a real bug found while preparing to run `backfill_full_reparse` against the production backlog: at a measured ~0.5s/draft, the full 15,000+-row backlog would take roughly 2 hours in one HTTP request, so the intended usage is several bounded `limit=` calls — but the query ordered by `created_at` with no offset, and a row whose re-parse reproduced the same confidence (including every genuinely-unresolvable draft, not just buggy ones) was never written to at all. That meant the exact same oldest-N rows came back on every subsequent call with the same `limit`, forever — a bounded batch could never advance past the stuck rows to reach later, actually-fixable drafts. Fixed by ordering `SELECT` by `d.updated_at` instead of `d.created_at`, and — `dry_run=False` only — always bumping `updated_at` on a no-op row via a lightweight `UPDATE drafts SET updated_at = now() WHERE draft_id = %s`, even when nothing else changes. One new test (`scripts/hermes-850-signature-company-fill-check.py`) confirms a stuck draft's `updated_at` moves while its payload stays untouched.
- `2d7c24cd` is a performance follow-up, found while about to run the now-fixed batch against production: re-parsing itself is ~0.5s/draft of pure CPU with no DB access, but a 100-row apply batch (`dry_run=False`) didn't finish in 180 seconds, because every row opened its own `with cursor()` for the `UPDATE` and, separately, its own for `record_field_provenance`'s `INSERT` — two commit/network round-trips per row on top of the parsing cost. `backfill_full_reparse` (`app/drafts/service.py`) is now split into two phases: phase 1 does all the CPU-bound re-parsing with zero database access, collecting what needs writing; phase 2 (only when `dry_run=False` and there is something to write) opens one transaction and performs every changed-row `UPDATE`, every no-op row's `updated_at` touch (via `executemany`), and every provenance insert against it, committing once at the end. `record_field_provenance` (`app/email_parsing/provenance.py`) gained an optional `cur` parameter so it can reuse that already-open cursor instead of opening its own; every pre-existing call site is unaffected, since not passing `cur` keeps the exact prior behavior (open one, commit, close).
- `8917a375` fixes a real production incident, live-debugged per the commit message: after a container restart, a request landing on `parse_email_signature`'s `is_pure_forward` → NER fallback path paid the full cost of loading `en_core_web_sm` synchronously — measured at well over ten minutes cold on this deployment's hardware, indistinguishable from a hang (other endpoints kept serving normally throughout, and `pg_stat_activity` showed nothing running in the database, which is what pointed at this rather than a DB lock). Adds `warm_ner_pipeline()` (`app/email_parsing/signature.py`), a thin wrapper forcing the `lru_cache`'d `_get_ner_pipeline()` load, called from a new FastAPI `@app.on_event("startup")` handler in `app/main.py` alongside the existing schema-init one — moving the cost to container start, where a slow boot is expected, instead of onto whatever ordinary request happens to hit the NER path first. Safe in CI: if spaCy or its model isn't installed, `_get_ner_pipeline()` already returns `None` without raising, so the new startup call is a no-op there. One new test (`scripts/hermes-850-email-signature-check.py`) confirms `warm_ner_pipeline()` is safe to call (twice, to exercise the `lru_cache` hit) with or without the model installed.

All three commits are confirmed green via the GitHub Actions API (`.github/workflows/hermes-regression.yml`, "Hermes regression suite"): `e4071bfa` → run [61](https://github.com/jobfynder/hermes/actions/runs/34281975255) `conclusion: success`; `2d7c24cd` → run [63](https://github.com/jobfynder/hermes/actions/runs/34283401202) `conclusion: success`; `8917a375` → run [65](https://github.com/jobfynder/hermes/actions/runs/34287283164) `conclusion: success`. Live re-verification on INTEL-1 was not attempted: no SSH credentials configured in this environment. This does not change HERMES-850's maturity band or any dimension grade — the same reasoning as v1.10 applies (Testing was already 🟢; Deploy/Integration stay 🟡 because live provider wiring is unaffected).

None of `backfill_full_reparse`'s pagination/performance fixes or `warm_ner_pipeline()` have been added to `hermes/HERMES-850-email-parsing-foundation.md` or `hermes-api-route-inventory.md`, which remain frozen as of the module's 2026-08-21 closure — this paragraph is the current record for these changes until/unless those files are revised.
