...
**2026-09-12 (v1.11):** Scheduled docs-sync run. Thirteen new commits on `jobfynder/hermes` `main` since v1.10 (`5f239f79..061aee2`), spanning HERMES-850 and HERMES-400 (no new commits on `jobfynder-infra` `main`; deployed COMM branch `feature/comm-telegram-message-chunking` unchanged at `0c33580`). PR #11 (open as of v1.10) is merged — `e4071bf`/`2d7c24c` via merge commit `eb1ad1b`. Highlights: durable, resumable, pausable backfill jobs replace the old synchronous backfill endpoints (`342cd49` + schema-race fixes `5c0dcea`/`9cd5045` + a `review-reparse` job kind in `73bb5d7`); a real production incident fixed where a cold NER-model load on a fresh container looked exactly like a hung API request (`8917a37`); the review dashboard's payload dropped from ~20 MB/2.69 s to ~27 KB/0.34 s via server-side pagination (`3e57878`); more email-title recovery fixes and a `POST /drafts/backfill/jobs` `review-reparse` kind (`73bb5d7`); a taxonomy-candidate lookup indexed for scale (`15c0d26`); and two taxonomy-integrity fixes (`b63bdd7`, `061aee2`) that disable automated LLM-based skill/job-title/boilerplate auto-approval platform-wide and add a technical-only skill filter — closing a real gap in the "unknown terms are never auto-approved" rule §4's HERMES-400 evidence already claimed was built. Also fixed: a real bug where a successful LLM fallback silently overwrote a document's original extracted text with the LLM's own structured-output dict instead of just merging into `structured_data` (`b5a245c`), plus two more call sites now explicitly pass `skip_llm_fallback=True` so deterministic email parsing can no longer silently reach an LLM. **Regression found by this run, not fixed (a docs-sync task doesn't push code fixes): `main`'s CI ("Hermes regression suite") has been red on every push since `73bb5d7` (2026-09-10 19:30 UTC) through the current HEAD `061aee2` (2026-09-11 17:19 UTC) — 5 consecutive commits, confirmed via the GitHub Actions API and by reading the actual job logs, not just the conclusion field.** See the HERMES-850 section below for the root cause and what it does and doesn't put in doubt; HERMES-850's Testing dimension is downgraded 🟢→🟡 below until it's fixed. No SSH credentials are configured in this environment — live verification on INTEL-1/COMM-1 was not attempted, same as every prior cycle.
...
**Update (2026-09-12, v1.11) — a real gap in the "never auto-approved" safety rule, closed.** `b63bdd7` and `061aee2` found and fixed a gap in the claim above: `scripts/hermes-900-daily-taxonomy-triage.py` (an LLM-classifier-driven daily job, despite its script-name prefix; it is taxonomy tooling, not a HERMES-900 Role Intelligence Package, which per the HERMES-900 section below still has zero implementation) could auto-approve or auto-reject `skill`/`job_title`/`boilerplate_line` candidates via `classify_job_title_family`/`generate_skill_description` calling out to LiteLLM. `b63bdd7` disabled this for `skill` outright (`triage()` now returns `skipped_reason: "automatic_skill_approval_disabled"` for that signal type, no classifier call at all) after prose/metadata fragments ("Advanced", "Recommendations", email addresses, unbalanced parentheses) were found already approved as canonical skills; `061aee2` extended the same disable to `job_title` and `boilerplate_line` (`skipped_reason: "automatic_taxonomy_llm_triage_disabled"`) and made `generate_skill_description`/`classify_job_title_family` require an explicit `allow_llm=True` the daily triage no longer passes, rather than just checking `litellm_configured()`. New `app/understanding/taxonomy/skill_quality.py` (`skill_noise_reason`) is a denylist/pattern check (standalone prose, sentence fragments, unbalanced parentheses, emails/URLs, and — added in `061aee2` — non-technical "professional skills" like leadership/mentoring/stakeholder-management/generic "communication"/"collaboration") wired into both `add_canonical_skill` (`loader.py`) and `approve_taxonomy_candidate` (`candidates.py`), so a noise term is rejected even on manual approval, not just at the automated-triage step. `b63bdd7` also disables fuzzy matching for any already-approved learned candidate (`fuzzy_match_terms` in `app/understanding/parsers/skills.py`) — a short, unvetted learned term matching a substring anywhere in an email body was judged too weak a signal — and fixes `RELAY_FROM_BLOCK_RE` (`app/email_parsing/signature.py`) to tolerate blank lines between a relay's "From:"/name/company/email lines, which the original regex (added in v1.10, `284b3bce4`) required to be contiguous. `PARSER_VERSION` bumped to `basic_local_parser_v2_skill_evidence`. Covered by 10 new tests in `tests/test_skill_quality.py`, `tests/test_relay_spacing.py`, and `tests/test_deterministic_taxonomy.py`, all passing per the 51-test unittest run confirmed in the HERMES-850 section above (`hermes-400-taxonomy-validation.py`/`hermes-400-signal-extraction-check.py`, the separate legacy check scripts, have not run since `73bb5d7` — see that section's regression note).

**Update (2026-09-12, v1.11) — taxonomy lookup/index performance, `342cd49` and `15c0d26`.** `342cd49` adds `cache_by_taxonomy` (`app/understanding/taxonomy/loader.py`): derived indexes (`build_skill_alias_index`, `build_title_alias_index`, and the signal-candidate builders in `signals.py`) are now cached and only rebuilt when the underlying taxonomy source objects actually reload, rather than the "deliberately uncached... microseconds, not worth it" comment the code previously carried — the docstring itself is corrected in place, not just the behavior. `signals.py`'s `_extract_candidates` also adds a cheap ASCII literal-substring prefilter before compiling/running each candidate's regex (`_safe_phrase_pattern`, now `lru_cache`d). Separately, `15c0d26` adds a partial expression index (`idx_taxonomy_candidates_pending_loose` on `app/runtime/db.py`'s schema) so `_find_loosely_matching_pending_candidate` no longer table-scans every pending row per observed term — a query plan assertion in the new `tests/test_candidate_lookup.py` confirms the index is actually used (`EXPLAIN` output contains the index name), not just present in the schema.
...
**Update (2026-09-12, v1.11) — PR #11 merged, backfill made durable and resumable.** PR #11 landed as `e4071bf` (ordering the backfill query by `updated_at` instead of `created_at`, and touching `updated_at` even on a no-op row, so a bounded `limit=`-ed call actually advances through the 15,000+-row backlog instead of re-selecting the same permanently-stuck oldest rows forever), followed on the same branch by `2d7c24c` (batching every row's UPDATE/provenance-INSERT for a call into one transaction instead of one round-trip per row — a 100-row apply batch that previously didn't finish in 180s) and `8917a37` (unrelated fix bundled on the same branch, see below); merged to `main` via `eb1ad1b`.

`342cd49` (`Fix Graph heartbeat starvation and add durable resumable backfills`) goes further and replaces the synchronous backfill endpoints entirely with a durable job system, because even the batched version above still risks a very long blocking HTTP request over the full backlog:
- New `app/drafts/backfill.py`: `create_job`/`get_job`/`control_job`/`run_batch` back two new tables (`draft_backfill_jobs`, `draft_backfill_items`, `app/runtime/db.py`). A job is created with a client-generated UUID (idempotent retry on an uncertain HTTP result), a `kind` (`full-reparse` or `signature-company-fill`), `dry_run`, `batch_size` (1-100), and an optional `limit`; only one unfinished job is allowed system-wide (`pg_advisory_xact_lock`-serialized). A dedicated `hermes-backfill-worker` container (new `docker-compose.yml` service, its own Docker healthcheck watching a heartbeat file) claims and processes bounded batches: parsing happens with zero database access, then one transaction does every write, provenance insert, and progress update for the batch. Pause/resume/cancel are supported; a crash before commit leaves the batch pending, not partially applied.
- New endpoints (`app/routers/drafts.py`, `drafts:publish`/`drafts:read` permissions): `POST /drafts/backfill/jobs`, `GET /drafts/backfill/jobs`, `GET /drafts/backfill/jobs/{job_id}`, `POST /drafts/backfill/jobs/{job_id}/{pause|resume|cancel}`. The old `POST /drafts/backfill/signature-company-fill` and `POST /drafts/backfill/full-reparse` endpoints are capped to `dry_run=True` previews only (`limit` bounded to 1-20); calling either with `dry_run=False` now returns 409 pointing callers at the durable job endpoints.
- Same commit also fixes a real bug in Microsoft Graph message fetching (`app/providers/microsoft_graph/service.py`): `fetch_graph_message` previously swallowed every HTTP error identically; it now distinguishes 404/410 (dead-lettered, not retried — the reference may have moved) from 429 (retried respecting `Retry-After`, capped at 1 hour) from other transport failures (bounded retry), and the Graph consumer now parses each delivery in a spawned child process so a slow parse can't starve the RabbitMQ heartbeat — the literal problem the commit title names.
- Also adds `app/runtime/db.py`'s `transaction()` context manager, now wrapping `process_channel_intake` (`app/channels/service.py`) so the transport dedupe key and the draft it protects commit atomically — a crash between the two can no longer leave one without the other.
- Taxonomy-side performance changes bundled in the same commit are evidence for HERMES-400, not HERMES-850 — see the HERMES-400 section below.
- `5c0dcea` and `9cd5045` fix a schema-initialization race the new multi-process worker setup exposed: concurrent `CREATE TABLE IF NOT EXISTS` calls from the API and worker starting together could still race PostgreSQL's type catalog. Fixed with an advisory lock (`pg_advisory_xact_lock`) around `init_schema()`, and a second advisory lock (`pg_advisory_xact_lock_shared`) taken by every ordinary transaction/intake so a schema migration waits for in-flight work to finish before taking its exclusive lock, rather than racing it. Both come with a concurrency test in `tests/test_production_reliability.py` exercising 4 parallel `init_schema()` calls and a transaction that must complete before a queued schema init proceeds.
- `73bb5d7` adds a third job kind, `review-reparse` (needs_review-only, excluding drafts with a recorded human correction) — see `docs/email-review-repair.md`.
- New docs file `docs/graph-production-operations.md` records a write-inclusive benchmark from 2026-09-09: 5/20/100-row batches completed in 6.411/6.638/24.160 seconds with every row updated and zero failures.

**Update (2026-09-12, v1.11) — review dashboard server-side pagination (`3e57878`).** The parsed-email review dashboard previously fetched the full drafts list client-side; `docs/dashboard-review-operations.md` records a same-day production measurement (2026-09-10) of the old endpoint returning 15,117 records (~20.35 MB) in 2.69s versus the new `GET /drafts/page` (new `app/drafts/listing.py`) returning a 50-row first page (~27 KB) in 0.34s, with filtering/search/counts done in PostgreSQL. Same commit adds `POST /drafts/review/reconcile-ready` (new `app/drafts/review_rules.py`, `drafts:publish` permission, `dry_run=True` default): repairs a draft stuck at `needs_review` only when both intake and per-record parser confidence are ≥0.7, there are no warnings/errors, `document_kind` matches, and no `reviewer_correction`/`recruiter_correction` provenance exists — it never publishes anything, and records the prior status and the rule that fired in the draft's own metadata in the same transaction.

**Update (2026-09-12, v1.11) — more email-title recovery, `73bb5d7`.** Continuing the `requires_review`-reduction work from v1.9/v1.10: `_extract_heading_title` (`app/email_parsing/parsers.py`) recovers a title from a flattened table/body heading (a "Title:" label on its own line, or a role-keyword line immediately followed by a location line) rather than only a same-line label; `extract_probable_title` (`app/understanding/parsers/basic.py`) now handles a leading `.NET` (previously excluded by its own character class), parenthesized text, HTML-entity-escaped subjects, and an "urgent"/"immediate hiring" prefix, plus five more title keywords (designer, researcher, technician, scientist, specialist, tester); `_split_requirement_sections` now strips a forwarded-header block and a job-description footer before splitting, and requires a repeated Role/Position label's value to itself look like a title (not work-mode prose or a screening step) before treating it as a new posting boundary; contact parsing (`app/understanding/parsers/contact.py`) recognizes "Any Visa"/"all visa types accepted" as an explicit work-authorization value instead of dropping it. `docs/email-review-repair.md` records the intent and rollback plan.

**Update (2026-09-12, v1.11) — deterministic email parsing enforced model-free, plus a real raw-text-corruption bug fixed (`b5a245c`).** Two of the four call sites that changed in this commit close a real gap in the "deterministic, no-LLM" claim this document makes about email parsing: `_process_channel_intake` (`app/channels/service.py`, for `request.channel == "email"`) and `parse_requirement_email` (`app/email_parsing/parsers.py`) now explicitly pass `skip_llm_fallback=True` into `understand_document` (the parameter itself already existed, added in an earlier, pre-v1.7 commit `28a56e2` — this commit is what actually wires these two call sites to use it), so a low-confidence deterministic parse of an email can no longer silently trigger an LLM fallback call the way it previously could. Separately, and unrelated to the deterministic-boundary change, this commit fixes a real bug in `build_understanding_result` (`app/understanding/service.py`): when an LLM fallback *did* fire (on non-email document kinds, where fallback is still allowed), the code reassigned its own `extracted` local variable to the LLM's structured-output dict — the same variable used moments later as `result.extracted_text=extracted` — silently replacing the original parsed document's raw text with the LLM's extraction object instead of only merging it into `structured_data`. Fixed by renaming the LLM output to its own `llm_extracted` variable. New `tests/test_email_fallback_boundary.py` (2 tests, part of the 51-test suite confirmed passing above) covers both: that `parse_requirement_email` calls `understand_document` with `skip_llm_fallback=True`, and that a successful fallback leaves `result.extracted_text.text` equal to the original input text.

**Regression found by this sync run (2026-09-12) — `main`'s CI has been red since `73bb5d7`, not just code-verified as green.** Reading the actual GitHub Actions job logs (not just the `conclusion` field) for every push to `main` since v1.10 shows the "Hermes regression suite" workflow's `backend` job failing identically on all 5 commits from `73bb5d7` (run [69](https://github.com/jobfynder/hermes/actions/runs/34520846850), 2026-09-10 19:30 UTC) through the current HEAD `061aee2` (run [73](https://github.com/jobfynder/hermes/actions/runs/34626987597), 2026-09-11 17:19 UTC) — `15c0d26`, `b63bdd7`, and `b5a245c` in between (runs 70-72) fail the same way. The 51-test `python -m unittest` suite that runs first in the same CI step passes cleanly every time (includes the new tests from all thirteen commits above: `test_dashboard_listing`, `test_review_rules`, `test_email_review_regressions`, `test_candidate_lookup`, `test_relay_spacing`, `test_skill_quality`, `test_email_fallback_boundary`, `test_deterministic_taxonomy`) — the evidence cited above for those specific fixes stands on its own. What fails is the older, separate `scripts/hermes-850-email-parsing-check.py`, in a pre-existing regression test unrelated to this run's changes: `test_position_ordinal_label_is_never_read_as_the_job_title` expects a "Position: 1 / Title: real title A ... Position: 2 / Title: real title B" vendor email to split into two records with titles `["...A", "...B"]`; since `73bb5d7`'s new boundary check in `_split_requirement_sections` (which correctly requires a repeated label's value to itself look like a title before starting a new section, so a bare ordinal "1"/"2" no longer counts), the email collapses into one section and the test now gets back only `['Java Developer with NoSQL and State Client Exp']` — the first job's title, with the second job's content and title lost into the first section rather than split out. This is a real, unfixed regression in `73bb5d7`'s own change, confirmed identically on the current `main` HEAD, not a flaky or environment-specific failure. Because the check-script step runs as a `bash -e` loop over 18 scripts and stops at the first failure, no push since `73bb5d7` has actually executed `hermes-850-email-signature-check.py`, `hermes-850-gmail-graph-provider-check.py`, `hermes-850-live-fetch-check.py`, `hermes-850-provenance-check.py`, `hermes-850-review-endpoints-check.py`, `hermes-850-self-learning-classification-check.py`, `hermes-850-signature-company-fill-check.py`, `hermes-850-sender-resolution-check.py`, `hermes-400-taxonomy-validation.py`, `hermes-400-signal-extraction-check.py`, `hermes-450-channel-intake-check.py`, or `hermes-900-spam-taxonomy-check.py` — their last known-passing state is the one recorded in v1.10 (run 59, 2026-09-08), not reconfirmed since. This does not invalidate the durable-backfill, dashboard-pagination, or taxonomy-integrity evidence above (all covered by the passing unittest suite or reviewed directly against the diff in this document), but it does mean HERMES-850's multi-posting split logic has an open, live regression on `main` as of this run. HERMES-850's Testing dimension is downgraded 🟢→🟡 in the status matrix above until CI is green again. No fix is pushed by this run — a scheduled docs-sync task updates documentation, not code; this is flagged here and in the priority matrix ([§8](#8-priority-matrix)) for a human or a separate engineering session to act on.

None of the durable-backfill job system, the dashboard-pagination endpoints, the title-recovery fixes, or the taxonomy-integrity changes documented above (v1.11) have been added to `hermes/HERMES-850-email-parsing-foundation.md`, `hermes-api-route-inventory.md`, or `hermes/HERMES-400-taxonomy-signal-intelligence.md`, which remain frozen as of their original closure dates — these paragraphs are the current record for these changes until/unless those files are revised.
...
| **P0** | Fix the live CI regression on `hermes` `main` (`test_position_ordinal_label_is_never_read_as_the_job_title`) | Found 2026-09-12 (v1.11): every push to `main` since `73bb5d7` (2026-09-10) has failed the "Hermes regression suite" workflow, and the failing check-script loop's `set -e` means a dozen other HERMES-850/400/450/900 check scripts haven't actually run since. Root cause is `73bb5d7`'s own boundary-validation change in `_split_requirement_sections`; see the HERMES-850 section above for the exact mechanism. Not fixed by this run — a docs-sync task doesn't push code. |
