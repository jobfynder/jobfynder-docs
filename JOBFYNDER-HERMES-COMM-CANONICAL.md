...
**Update 2026-09-07 (commit `8319c43f` on `jobfynder/hermes`).** The resume parser gained deterministic extraction of name, location, summary, structured experience (company/title/dates/location/description), structured education (institution/degree/field/year), and certifications — `ResumeStructuredData` had none of these fields before this commit. Phone extraction also gained international-number support (a labeled-field regex, then the `phonenumbers` library) where previously only US-shaped numbers matched. This closes part — the resume-field half, not the JD-field half — of the "more resume/JD fields" gap noted below. See `hermes/HERMES-200-understanding-foundation.md` for the full evidence trail. No live re-verification was performed for this entry — no SSH credentials were configured in the environment that produced it.

**Gap.** File itself lists open "Next" items: bigger taxonomy, more resume/JD fields (resume side partially addressed 2026-09-07, see update above; JD side untouched), real Unstructured.io integration, Langfuse/Promptfoo/Great Expectations evaluation — none confirmed done as of this document.
...
- **2026-09-07 update (commit `8319c43f`):** a local static prompt-fallback catalog was reintroduced for two prompt IDs (`jf.resume.parse`, `jf.onboarding.profile-import.extract`) already present in the live 38-prompt Langfuse catalog — resilience for a Langfuse outage, not a registry redesign. `GET /prompts/registry`'s `registry_version` field changed in code from `hermes_langfuse_prompt_registry_v1` to `hermes_prompt_registry_v1` as a result; `prompt_count` is unaffected (still 38). See `hermes/HERMES-750-litellm-prompt-runtime-foundation.md` §3/§7/§8. No live re-verification was performed for this entry.
