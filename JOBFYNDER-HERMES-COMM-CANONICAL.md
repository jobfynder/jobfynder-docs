...
**2026-09-08 (v1.11):** Scheduled docs-sync run. Two new commits on `jobfynder/hermes` `main` since v1.10 (`5f239f79..2d7c24cd`), both HERMES-850, continuing the `backfill_full_reparse` work v1.10 introduced: `e4071bfa` is PR [#11](https://github.com/jobfynder/hermes/pull/11) (`fix/hermes-850-full-reparse-batch-progress`, flagged as open and undocumented in v1.10) merged to `main` — fixes bounded `limit=`-ed calls to `backfill_full_reparse` so they actually advance through the backlog instead of re-examining the same oldest rows forever. `2d7c24cd` (`perf/hermes-850-full-reparse-single-transaction`) then batches that same backfill's database writes into one transaction instead of one round-trip per row, fixing a measured-in-production timeout (a 100-row apply batch didn't finish in 180s). Both commits' pushes to `main` confirmed green via the GitHub Actions API ("Hermes regression suite" runs [61](https://github.com/jobfynder/hermes/actions/runs/34281975255) and [63](https://github.com/jobfynder/hermes/actions/runs/34283401202), both `conclusion: success`). See the HERMES-850 section below for the full evidence trail. No new commits on `jobfynder-infra` `main` — its `main` HEAD is unchanged at `2622899`, and the deployed COMM branch (`feature/comm-telegram-message-chunking`) is unchanged, checked directly against the branch this run — its tip is still `0c33580`. Live verification skipped: no SSH credentials configured in this environment.
...
**Update (2026-09-08, v1.11).** PR [#11](https://github.com/jobfynder/hermes/pull/11) (flagged just above as open and undocumented) merged to `main` as squash-commit `e4071bfa` (the PR's own head was `ba7ef0c`). It fixes `backfill_full_reparse` (`app/drafts/service.py`, added in `f1dd1ce42`, v1.10): the query powering bounded `limit=`-ed calls ordered by `created_at` with no offset, and a row that turned out unchanged (including every genuinely-unresolvable draft, not only buggy ones) was never written to at all — so the exact same oldest-N rows came back on every subsequent call with the same `limit`, forever, and a bounded batch could never advance past a large contingent of permanently-stuck rows to reach later, actually-fixable drafts. Per the commit's own account, this mattered because the real backlog is 15,000+ rows and re-parsing measures ~0.5s/draft, so a single unlimited call would block roughly 2 hours in one HTTP request — too risky to just run, making the several-bounded-calls usage pattern load-bearing rather than optional. Fixed by ordering by `updated_at` instead, and (`dry_run=False` only) always bumping `updated_at` even on a no-op row, so every row a bounded batch examines moves to the back of the queue whether it needed a real fix or not. New test verifies a stuck (unchanged) draft's `updated_at` moves while its payload stays untouched.

`2d7c24cd` (`perf/hermes-850-full-reparse-single-transaction`) is a follow-up performance fix to the same function, measured against production after PR #11 was deployed per its own commit message: re-parsing itself is CPU-bound (~0.5s/draft, no DB access), but a 100-row apply batch didn't finish inside a 180s window, because every row opened its own database cursor for the UPDATE and a separate one for `record_field_provenance`'s INSERT — each its own commit/network round-trip. Splits the work into two phases: phase 1 does all CPU-bound re-parsing with zero database access; phase 2 (only when `dry_run=False` and there's something to write) opens one transaction and performs every UPDATE, every no-op row's `updated_at` touch, and every provenance insert against it, committing once at the end. `record_field_provenance` (`app/email_parsing/provenance.py`) gains an optional `cur` parameter to reuse an already-open cursor instead of opening its own; every existing call site is unaffected since not passing it keeps the exact prior behavior.

Both commits' pushes to `main` are confirmed green via the GitHub Actions API (`Hermes regression suite` workflow): `e4071bfa` → run [61](https://github.com/jobfynder/hermes/actions/runs/34281975255) `conclusion: success`; `2d7c24cd` → run [63](https://github.com/jobfynder/hermes/actions/runs/34283401202) `conclusion: success` (its PR run on `perf/hermes-850-full-reparse-single-transaction` also passed, run [62](https://github.com/jobfynder/hermes/actions/runs/34283033127)). Live re-verification on INTEL-1 was not attempted: no SSH credentials configured in this environment. Neither commit touches `hermes/HERMES-850-email-parsing-foundation.md` or `hermes-api-route-inventory.md`, which remain frozen as of the module's 2026-08-21 closure, and neither changes HERMES-850's maturity band or any dimension grade — this is bug-fixing and performance work on an already-🟢-Testing, already-🟡-Deploy/Integration module, not new scope.
