...
**2026-09-07 update.** Commit `8319c43f9fdd17b1f46937e64d458cc9a6dc3c13` on `jobfynder/hermes` adds a new `app/understanding/parsers/resume_sections.py` deterministic section splitter plus a `phonenumbers`-based international phone matcher (`app/understanding/parsers/contact.py`), wired into `parse_basic_structured_data()` so the resume parser now also returns name, location, summary, structured experience (company/title/dates/location/description), structured education (institution/degree/field/year), and certifications, in addition to the fields already listed above. Two new regression test files (`tests/understanding/test_resume_sections.py`, `tests/understanding/test_basic_resume_parse.py`) assert this against a real Jake-style resume fixture including an Indian mobile number. The LLM fallback path now merges in whichever of these fields the deterministic parser left empty (`merge_llm_extracted()`) without overwriting anything already found. Full detail in `hermes/HERMES-200-understanding-foundation.md`'s 2026-09-07 update section. Not live-re-verified — no SSH credentials for `jobfynder-intel-01` were available in the environment that made this update; graded on the commit diff only.

The same commit adds a local fallback copy of two already-existing Langfuse prompts (`jf.resume.parse`, `jf.onboarding.profile-import.extract`) in a new `app/prompt_runtime/registry.json`, served only when Langfuse can't supply that ID, and renames the `/prompts/registry` `registry_version` string from `hermes_langfuse_prompt_registry_v1` to `hermes_prompt_registry_v1`. This narrows the "not hardcoded" claim in `hermes/hermes-architecture-frozen-v1.md` §9 for those two IDs specifically — footnoted there and in `hermes/hermes-parsing-and-prompts-api-guide.md`. Does not change the live 38-prompt Langfuse count.

**Gap.** File itself lists open "Next" items: bigger taxonomy, more JD fields, real Unstructured.io integration, Langfuse/Promptfoo/Great Expectations evaluation — none confirmed done as of this document. (More resume fields — see the 2026-09-07 update above.)
