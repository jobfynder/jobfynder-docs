...
**Extended 2026-09-07** (`jobfynder/hermes` commit `8319c43f9fdd17b1f46937e64d458cc9a6dc3c13`, `feat(understanding): extract resume sections and international phones without an LLM`). Closes part of the "more resume fields" gap above: `app/understanding/parsers/resume_sections.py` (new) deterministically extracts full name, location, structured experience entries (company/title/start-end dates/location/description), structured education entries (institution/degree/field/year), and certifications from bulleted, dated resume text; `app/understanding/parsers/contact.py` gained an international phone pattern ahead of the prior US-only one. A new local, file-based prompt registry (`app/prompt_runtime/local_prompts.py` + `registry.json`) now backs `jf.resume.parse` and `jf.onboarding.profile-import.extract` as a fallback when Langfuse is unconfigured or unreachable — see the note under HERMES-750/775 below on the prompt-ID naming overlap this introduces. **Verified in this sync:** `jobfynder/hermes` was checked out at this commit and the three new test files (`tests/understanding/test_resume_sections.py`, `tests/understanding/test_basic_resume_parse.py`, `tests/prompt_runtime/test_local_resume_extract.py`) were run directly — 6/6 passed. Not re-verified against a live `/understanding/parse-file` call (no SSH credentials for INTEL-1 configured in this environment). JD-side field extraction is unchanged.
...
**Naming overlap introduced 2026-09-07 (commit `8319c43f` on `jobfynder/hermes`) — flagged so it isn't misread as contradicting item 5 above.** A new local, file-based prompt registry (`app/prompt_runtime/local_prompts.py` + `registry.json`) now backs `get_prompt()`/`list_prompts()` as a fallback, used when `LANGFUSE_SECRET_KEY`/`LANGFUSE_PUBLIC_KEY` are unset or a live Langfuse fetch fails. Its six entries include `resume_builder.summary_improve` and `resume_builder.bullet_rewrite` — the exact two prompt IDs item 5 says no longer exist **in the live Langfuse registry**. That finding is still accurate for Langfuse itself; these two IDs now additionally resolve through the new local fallback path instead, which the original HERMES-800 closure doc and the 2026-08-21 check-script fix did not anticipate. Not yet re-verified whether `hermes-750-prompt-runtime-check.py`/`hermes-800-foundation-check.py` (or a live `GET /prompts/registry` call) treat a locally-resolved prompt the same way as a Langfuse-resolved one — this needs checking before anyone relies on those check scripts to prove Langfuse coverage specifically.
