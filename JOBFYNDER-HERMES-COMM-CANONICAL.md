...
**2026-09-08 (v1.11):** Scheduled docs-sync run. Three new commits on `jobfynder/hermes` `main` since v1.10 (`5f239f79..8917a375`), all HERMES-850: PR #11 (flagged unmerged in v1.10) landed as `e4071bfa`; `2d7c24c` is a follow-up perf fix (production measured a 100-row apply batch not finishing in 180s after `e4071bfa` deployed) batching `backfill_full_reparse`'s writes into one transaction instead of one per row; `8917a375` fixes a real production incident (a fresh container's first NER-fallback request paying a 10+ minute cold `en_core_web_sm` load synchronously, indistinguishable from a hang) by warming the model at FastAPI startup instead. All three confirmed green via the GitHub Actions API on `main` (runs 61/63/65, `conclusion: success`). See the HERMES-850 section below for the full evidence trail. No new commits on `jobfynder-infra` `main`; the deployed COMM branch (`feature/comm-telegram-message-chunking`) is unchanged, still `0c33580`. No open PRs remain on `jobfynder/hermes`. Live re-verification on INTEL-1/COMM-1 was not attempted: no SSH credentials configured in this environment.
...
**Update (2026-09-08, v1.11).** PR #11, flagged above as open and unmerged, has landed on `main` as commit `e4071bfa62b468af1997da5f00434c77ba1a887b` (same fix described above: `backfill_full_reparse`'s query now `ORDER BY d.updated_at` instead of `d.created_at`, and — `dry_run=False` only — bumps `updated_at` even on a no-op row, a lightweight `UPDATE ... SET updated_at = now()` with no payload rewrite). Per the commit message, the bug this closes is real: without it, a bounded `limit=`-ed call (the only safe way to run this against the real ~15,000-row backlog, since the full pipeline measures ~0.5s/draft) would re-select the exact same oldest-`created_at` rows on every call forever, because a row whose re-parse reproduced the same confidence — including every genuinely-unresolvable draft, i.e. free text with nothing to extract — was never written to at all. A new test confirms a stuck draft's `updated_at` moves while its payload stays byte-for-byte untouched. Confirmed green via GitHub Actions: `.github/workflows/hermes-regression.yml` run [61](https://github.com/jobfynder/hermes/actions/runs/34281975255), `conclusion: success`. `fix/hermes-850-full-reparse-batch-progress` no longer appears among `jobfynder/hermes`'s open PRs as of this run.

A follow-up commit, `2d7c24cdc28ff6fef2b92f60abba627f571aa156` (`perf(drafts): batch full-reparse writes into one transaction instead of one per row`), reports in its own message that production, measured after `e4071bfa` deployed, showed a 100-row apply batch not finishing in 180 seconds — the re-parse itself is ~0.5s/draft with zero DB access, but every row was opening its own `with cursor()` (its own commit/network round-trip) for the `UPDATE`, and a separate one again for `record_field_provenance`'s `INSERT`. `backfill_full_reparse` is now split into a CPU-only phase 1 (re-parses every row, touches the database not at all) and, only when `dry_run=False` and there is something to write, a single phase-2 transaction covering every real `UPDATE`, every no-op row's `updated_at` touch, and every provenance insert, committed once at the end. `record_field_provenance` (`app/email_parsing/provenance.py`) gained an optional `cur` parameter to reuse that already-open cursor instead of opening its own; every pre-existing call site is unaffected, since none of them pass it. Confirmed green via GitHub Actions: run [63](https://github.com/jobfynder/hermes/actions/runs/34283401202), `conclusion: success`. The 180-second production measurement is the commit author's own account, not independently re-measured in this run — live re-verification on INTEL-1 was not attempted, no SSH credentials configured in this environment.

Separately, `8917a375f0874a4ce434f7566c2c8163613429ec` (`fix(startup): load the NER model at app startup, not on the first request that needs it`) fixes a real production incident the commit message describes as live-debugged: on a freshly-restarted container, whichever request first landed on `parse_email_signature`'s `is_pure_forward` → NER fallback path (`app/email_parsing/signature.py`'s `_get_ner_pipeline`, `lru_cache`'d per-process) paid the full cost of loading `en_core_web_sm` synchronously — measured at well over ten minutes cold on this deployment's hardware, indistinguishable from a hang (other endpoints kept serving normally throughout, and `pg_stat_activity` showed nothing running, which is what pointed at this rather than a DB lock or a runaway regex). Adds `warm_ner_pipeline()` (forces the `lru_cache`'d load) and calls it from a new FastAPI startup event in `app/main.py`, alongside the existing schema-init one — moving the cost to container start instead of a live request. Safe in CI: if `spacy`/`en_core_web_sm` isn't installed, `_get_ner_pipeline` still returns `None` without raising, so the new startup call is a no-op there, matching prior behavior. Confirmed green via GitHub Actions: run [65](https://github.com/jobfynder/hermes/actions/runs/34287283164), `conclusion: success`.

None of these three commits have been added to `hermes/HERMES-850-email-parsing-foundation.md` or `hermes-api-route-inventory.md`, which remain frozen as of the module's 2026-08-21 closure — consistent with v1.9/v1.10, this paragraph is the current record for these changes until/unless those files are revised. This does not change HERMES-850's maturity band or any dimension grade — live provider wiring (the reason Deploy/Integration stay 🟡) is unaffected.
