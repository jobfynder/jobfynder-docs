...
**Update (2026-09-07).** Commit `8319c43` on `jobfynder/hermes` `main` adds deterministic resume-section parsing (new file `app/understanding/parsers/resume_sections.py`) — the resume parser now also extracts name, location, summary, structured experience (company/title/start-end date/location/description), structured education (institution/degree/field/year), and certifications, all still local/deterministic (no LLM). `app/understanding/parsers/contact.py`'s `extract_phone()` now also matches labeled phone fields and non-US numbers via the `phonenumbers` library, not just the US 3-3-4 pattern. Verified by new tests against a "Jake-style" resume fixture (`tests/understanding/test_resume_sections.py`, `tests/understanding/test_basic_resume_parse.py`), including a `+91` Indian mobile number. See `hermes/HERMES-200-understanding-foundation.md` "Update 2026-09-07" for the full field list. Not live-verified against INTEL-1 — no SSH credentials configured in this environment.

**Gap.** File itself lists open "Next" items: bigger taxonomy, more resume/JD fields, real Unstructured.io integration, Langfuse/Promptfoo/Great Expectations evaluation. The 2026-09-07 update above addresses the resume-fields half of "more resume/JD fields" — JD-field expansion, bigger taxonomy, real Unstructured.io integration, and the evaluation tooling remain open.
...
**Update (2026-09-07).** Commit `8319c43` on `jobfynder/hermes` `main` adds a local fallback prompt registry (`app/prompt_runtime/local_prompts.py`) so `jf.resume.parse` and `jf.onboarding.profile-import.extract` stay available if Langfuse is unreachable, merged into `list_prompts()`/`get_prompt()` with Langfuse taking precedence when configured. This also renamed the `registry_version` field `list_prompts()` returns, from `hermes_langfuse_prompt_registry_v1` back to `hermes_prompt_registry_v1` — same string as the original 2026-07-10 static 4-prompt registry, but now meaning something different (a merged local+Langfuse registry, still 38 prompts). See `hermes/HERMES-750-litellm-prompt-runtime-foundation.md` §11 for the full evidence trail. Not live-verified — no SSH credentials configured in this environment.
