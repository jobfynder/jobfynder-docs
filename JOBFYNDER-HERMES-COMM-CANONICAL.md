...
**Gap.** File itself lists open "Next" items: bigger taxonomy, more resume/JD fields, real Unstructured.io integration, Langfuse/Promptfoo/Great Expectations evaluation. **Partially resolved 2026-09-07** for the resume side of "more resume/JD fields": commit `8319c43f9fdd17b1f46937e64d458cc9a6dc3c13` on `jobfynder/hermes` `main` added a deterministic resume-section parser (`app/understanding/parsers/resume_sections.py`) that extracts name, location, summary, structured experience entries (company/title/dates/location/description), structured education entries (institution/degree/field/year), and certifications — fields the resume parser previously did not return at all — plus international phone-number support (`phonenumbers`-backed, alongside the existing US-shaped regex) in `app/understanding/parsers/contact.py`. See `hermes/HERMES-200-understanding-foundation.md` for the full evidence trail and its new tests. JD-field extraction, the taxonomy size, Unstructured.io, and the evaluation-framework items remain untouched by this commit and are still open.
