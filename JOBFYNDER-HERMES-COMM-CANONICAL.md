...
**2026-09-12 (v1.11):** Scheduled docs-sync run. Thirteen new commits on `jobfynder/hermes` `main` since v1.10 (`5f239f79..061aee28`), spanning HERMES-850 and HERMES-400. On HERMES-850: PR #11 (left open at v1.10) landed via merge commit `eb1ad1b8`, bringing in `e4071bfa` (fixes the bounded full-reparse batch-progress bug), `2d7c24cd` (batches full-reparse writes into one transaction), and `8917a375` (loads the NER model at container startup instead of on first request, closing a real incident where a cold load looked indistinguishable from a hang). `342cd499` fixes Graph-consumer heartbeat starvation and adds a whole new durable, resumable, pausable/cancelable backfill-job system (`POST /drafts/backfill/jobs` + job-control endpoints) that supersedes the synchronous dry-run-only backfill endpoints recorded in v1.9/v1.10; `5c0dceab`/`9cd50459` harden the schema-initialization serialization it introduced. `3e578788` replaces the review dashboard's full-table load with a paginated `GET /drafts/page` and adds a conservative `POST /drafts/review/reconcile-ready` repair endpoint. `73bb5d7b` adds a `review-reparse` backfill kind and fixes several email-title recovery bugs — but also introduces a real, still-open regression: the "Hermes regression suite" CI check has failed identically on every push since (`73bb5d7b` through `061aee28`, 5 consecutive failures, confirmed via job logs, not a flake), and because the CI script loop aborts on first failure, every test file added across this entire commit range has never actually run in CI. HERMES-850's Testing dimension moves 🟢→🔴 pending a fix; see the HERMES-850 section and new P0 priority-matrix item. On HERMES-400: `15c0d260` indexes pending-candidate loose matching; `b63bdd72` and `061aee28` add a new skill-quality guardrail module that blocks non-technical/prose/fragment terms from ever reaching the taxonomy and hard-disable automatic LLM-based taxonomy triage. See the HERMES-850 and HERMES-400 sections below for the full evidence trail. No new commits on `jobfynder-infra` `main`, and no evidence the deployed COMM branch changed. **Flag for human attention:** this run's evidence (a live Graph consumer, a backfill worker, and a 15,117-record production review-dashboard measurement, all cited below) continues to sit uneasily next to `hermes/HERMES-850-email-parsing-foundation.md` §7's frozen 2026-08-21 claim that "no real email flows through this yet" pending OAuth credentials — as with v1.9/v1.10, this sync does not resolve that tension (the module doc remains frozen, out of scope for an automated sync) but is flagging it again since the gap between the frozen doc and the evidence keeps widening. Live re-verification on INTEL-1 was not attempted this run: no SSH credentials configured in this environment.
...
**Update (2026-09-10 to 2026-09-11, v1.11).** Three commits on `jobfynder/hermes` `main` tighten taxonomy-candidate quality and performance:

- `15c0d260` ("Index pending taxonomy candidate matching to unblock live email intake") replaces `_find_loosely_matching_pending_candidate()`'s full table scan over one signal type's pending rows — the code's own prior comment called this "fine at review-queue scale... not worth a schema change for" — with a new partial expression index, `idx_taxonomy_candidates_pending_loose` (`ON taxonomy_candidates (signal_type, regexp_replace(normalized_term, '[^a-z0-9]', '', 'g'), id) WHERE status = 'pending'`), and an indexed `SELECT ... ORDER BY id LIMIT 1` in place of fetch-and-filter-in-Python. 4 new tests in `tests/test_candidate_lookup.py`, including one that runs `EXPLAIN` and asserts the new index name appears in the plan.
- `b63bdd72` ("Recover spaced relay companies and prevent taxonomy noise promotion") widens `RELAY_FROM_BLOCK_RE` (`app/email_parsing/signature.py`) to tolerate up to 3 blank lines between the From:/name/company/email lines of a relay's rigid header block (previously required them immediately adjacent) — new `tests/test_relay_spacing.py`. Adds a new module, `app/understanding/taxonomy/skill_quality.py` (`skill_noise_reason()`), rejecting standalone prose/metadata terms (a fixed `NON_SKILL_TERMS` set — e.g. "receive," "employment type," "responsibilities"), sentence fragments ("or Microsoft Copilot," "experience with..."), unbalanced-parenthesis truncations, and contact/URL text from ever being promoted as a canonical or candidate skill. Wired into `add_canonical_skill()` (`loader.py`, raises `ValueError`) and `approve_taxonomy_candidate()` (`candidates.py`, returns `{"approved": false, "reason": "invalid_skill_name"}`). `fuzzy_match_terms()` (`app/understanding/parsers/skills.py`) now returns no matches at all for any skill whose `source` is `"taxonomy_candidate_approved"` — a learned candidate needs exact evidence, not a fuzzy substring hit against a whole email body. `scripts/hermes-900-daily-taxonomy-triage.py`'s `triage()` now hard-disables automatic LLM-based approval for `signal_type == "skill"` entirely, returning `skipped_reason: "automatic_skill_approval_disabled"` with zero classifier calls. `PARSER_VERSION` (`app/understanding/service.py`) bumped to `"basic_local_parser_v2_skill_evidence"`. New `tests/test_skill_quality.py`.
- `061aee28` ("Enforce technical-only taxonomy and deterministic enrichment defaults") extends `skill_noise_reason()` to also reject non-technical "professional/soft skill" terms (stakeholder management, leadership, mentoring, teamwork, interpersonal skills, "soft skills," technical writing, time management, problem solving, contract negotiations, and bare "communications"/"collaboration"/"presentations" without a technical qualifier), with explicit carve-outs for genuinely technical uses (Communication Protocols, Unified Communications, Microsoft Teams, Miro, SharePoint, GitHub, Confluence). Extends the daily-triage auto-approval ban from skill-only to also cover `job_title` and `boilerplate_line` signal types (`skipped_reason` renamed to `"automatic_taxonomy_llm_triage_disabled"`). `generate_skill_description()` (`descriptions.py`) and `classify_job_title_family()` (`title_family_classifier.py`) both gain an `allow_llm: bool = False` parameter — an LLM call now requires an explicit opt-in from the caller instead of firing automatically whenever `litellm_configured()` is true. 2 new tests confirm no LLM call fires by default on either path.

None of the new `idx_taxonomy_candidates_pending_loose` index, the `skill_quality.py` module, or the daily-triage auto-approval changes have been added to `hermes/HERMES-400-taxonomy-signal-intelligence.md`, which remains frozen as of its original closure — this paragraph is the current record for these changes until/unless that file is revised. Live re-verification on INTEL-1 (confirming the index is actually used in the live query planner, or that a live taxonomy-candidate approval attempt is actually blocked) was not attempted this run: no SSH credentials configured in this environment; the `EXPLAIN`-based test above is the strongest evidence available, and it runs against a disposable test database, not production.
...
**Update (2026-09-09 to 2026-09-11, v1.11).** PR #11 landed on `main` via merge commit `eb1ad1b8` ("Merge upstream backfill improvements and NER startup warmup"), bringing in three commits:

- `e4071bfa` — the fix PR #11 was opened for: `backfill_full_reparse`'s query ordered by `created_at` with no offset, and a row that turned out unchanged (including every genuinely-unresolvable draft) never got written to, so the exact same oldest-N rows came back on every subsequent bounded `limit=` call forever — a bounded batch could never advance past a large stuck contingent to reach later, fixable drafts. Fixed by ordering by `updated_at` instead and (`dry_run=False` only) always bumping `updated_at` on a no-op row too, so every examined row moves to the back of the queue whether or not it needed a real fix. New test verifies a stuck draft's `updated_at` moves while its payload stays untouched.
- `2d7c24cd` — `backfill_full_reparse`'s apply phase was measured unable to finish a 100-row batch in 180s even though re-parsing itself is only ~0.5s/draft pure CPU, because every row opened its own DB cursor separately for the UPDATE and for `record_field_provenance`'s INSERT. Split into a CPU-only re-parse phase with zero DB access, then (when there's something to write) one single transaction for every UPDATE/no-op-touch/provenance-insert, committed once. `record_field_provenance` (`app/email_parsing/provenance.py`) gained an optional `cur` parameter to reuse an already-open cursor; every existing call site is unaffected.
- `8917a375` — real production incident: after a container restart, a request landing on `parse_email_signature`'s NER fallback path paid the full cost of loading `en_core_web_sm` synchronously (measured well over ten minutes on this deployment's hardware), indistinguishable from a hang, while other endpoints kept serving normally. Adds `warm_ner_pipeline()` and a FastAPI startup event (`app/main.py`) that forces the load at container start instead of on the first request that needs it. No-op in CI where spaCy/the model isn't installed.

`342cd499` ("Fix Graph heartbeat starvation and add durable resumable backfills") is the largest single change in this window (873/-42 lines across 19 files) and adds a new file, `docs/graph-production-operations.md`, that is itself the evidentiary record for this change — quoted directly rather than restated:

- **Graph consumer reliability.** RabbitMQ I/O stays in the parent process; each delivery is parsed in a spawned child (prefetch 1); heartbeats stay active during parsing; processing has a 120-second deadline; a retry publication is confirmed before the original delivery is acknowledged; HTTP 404/410 references are preserved in the dead-letter queue rather than dropped; 429 responses respect `Retry-After` up to one hour. Intake database writes (transport dedupe key + draft) commit together, so a failed attempt rolls back cleanly for redelivery.
- **A new durable backfill-job system**, replacing the synchronous, dry-run-default endpoints recorded in v1.9/v1.10 (`POST /drafts/backfill/signature-company-fill`, `POST /drafts/backfill/full-reparse`): `app/drafts/backfill.py` (new), a dedicated `scripts/hermes-backfill-worker.py` process, and new `draft_backfill_jobs`/`draft_backfill_items` tables (`app/runtime/db.py`). `POST /drafts/backfill/jobs` takes a client-generated UUID `job_id` (safe to retry after an uncertain HTTP result; changing parameters for an existing UUID returns 409), `kind` (`full-reparse` or `signature-company-fill` at this commit), `dry_run`, `batch_size`, `limit`. `GET /drafts/backfill/jobs` lists recent jobs; `GET /drafts/backfill/jobs/{job_id}` returns durable counts/status/last-batch-duration/last-error-class; `POST /drafts/backfill/jobs/{job_id}/pause|resume|cancel` controls it. Only one unfinished job is allowed. The worker commits one transaction per bounded batch (draft changes, field provenance, item outcomes, progress together); a crash before commit leaves the batch pending, not partially applied. **The legacy synchronous endpoints from v1.9/v1.10 now only allow bounded dry-run previews (maximum 20); their mutation calls return 409 directing callers to the durable jobs API instead.**
- New verification: `scripts/hermes-graph-heartbeat-check.py` and `tests/test_production_reliability.py` (185 lines, new). Per the same doc: "Write-inclusive snapshot benchmarks on 2026-09-09 completed 5/20/100 rows in 6.411/6.638/24.160 seconds, with every row updated and zero failures."

`5c0dceab` ("Serialize schema initialization across API and workers") and `9cd50459` ("Coordinate schema migrations with active intake and backfill transactions") are small, code-only follow-ups (+15 and +23/-2 lines respectively, both confined to `app/runtime/db.py` and `tests/test_production_reliability.py`) hardening the schema-initialization concurrency guarantee `342cd499` introduced for the new multi-process worker topology (API + Graph consumer + backfill worker all sharing one schema-init path) — no new endpoints or behavior surface.

`3e578788` ("Paginate the review dashboard on the server and reconcile verified review flags") adds a new file, `docs/dashboard-review-operations.md`: `GET /drafts/page` now does filtering, search, counts, and pagination in PostgreSQL itself (new `app/drafts/listing.py`), page size defaulting to 50 and capped at 200, returning only the fields the table needs; the legacy summary endpoint remains available for compatibility. New `app/drafts/review_rules.py` backs `POST /drafts/review/reconcile-ready` (`drafts:publish`, `dry_run=True` default), which repairs a stale `needs_review` status only when intake and parser confidence both already meet the existing threshold, every record is clear with no warnings/errors, the document kind matches, and no human correction exists on the draft — it does not publish anything and does not fabricate a missing company/candidate name; the previous status and the rule that fired are recorded in draft metadata in the same transaction. Quoted directly from the same doc: "Production read-only measurements on September 10, 2026: the old list returned 15,117 visible records, about 20.35 MB, in 2.69 seconds before browser overhead. The new first page returned 50 records, about 27 KB, in 0.34 seconds. The needs-review page took 0.31 seconds; a Java search took 1.16 seconds." New tests: `tests/test_dashboard_listing.py`, `tests/test_review_rules.py`.

`73bb5d7b` ("Recover valid email titles and prevent false review records") adds a new file, `docs/email-review-repair.md`, and a third backfill kind, `review-reparse` (`app/drafts/backfill.py`, `app/routers/drafts.py`, the `draft_backfill_jobs` CHECK constraint), which snapshots only job-requirement drafts currently `needs_review`, excluding any with a recorded human correction — reuses the same durable worker, does not publish. Email-parsing fixes, all confirmed by the commit against real September 2026 review-backlog postings: a new `_extract_heading_title()` (`app/email_parsing/parsers.py`) recovers a title from a flattened table/body heading (requires a title label or an adjacent location line, never scans arbitrary prose); `extract_probable_title()` (`app/understanding/parsers/basic.py`) now HTML-unescapes input, strips "urgent"/"immediate" prefixes, and recognizes a leading `.NET`-style dot and five more title keywords (designer, researcher, technician, scientist, specialist, tester) — so ".NET Developer" and similar no longer fail to parse. `_split_requirement_sections()` now strips the forwarded-header block and the job-description footer before splitting, and requires a candidate section-starting value to itself look like a plausible, ≤100-character title — so a screening-step list, a signature's own "Role: Technical Recruiter" line, or a work-mode label ("Position: Onsite") occurring after a real job's Job Title/Position/Role label no longer creates a spurious second job-posting record. Adds "Any Visa" as a recognized work-authorization value (`app/understanding/parsers/contact.py`). 10 new tests in `tests/test_email_review_regressions.py`.

None of the durable backfill-job system, the paginated review dashboard, the `reconcile-ready`/`review-reparse` endpoints, or the email-title-recovery fixes above have been added to `hermes/HERMES-850-email-parsing-foundation.md` or `hermes-api-route-inventory.md`, which remain frozen as of the module's 2026-08-21 closure — these paragraphs are the current record for these changes until/unless those files are revised. Live re-verification on INTEL-1 (Docker health, RabbitMQ queue depths, an actual paused/resumed backfill job) was not attempted this run: no SSH credentials configured in this environment; all evidence above is code- and in-repo-doc-verified only.

**Open CI regression found this run — confirmed, not fixed here.** The "Hermes regression suite" GitHub Actions workflow has been red on every push to `main` since `73bb5d7b`, five consecutive failures through the current HEAD `061aee28` (runs [69](https://github.com/jobfynder/hermes/actions/runs/34520846850) through [73](https://github.com/jobfynder/hermes/actions/runs/34626987597), all `conclusion: failure`; every commit before `73bb5d7b` in this window is `conclusion: success`). Confirmed by pulling the actual job logs for both the first and last failing run — byte-for-byte the same failure, not a flake: `scripts/hermes-850-email-parsing-check.py::test_position_ordinal_label_is_never_read_as_the_job_title` raises `AssertionError: Must read the real title from 'Title:', never the ordinal from 'Position:', got ['Java Developer with NoSQL and State Client Exp']` — one title recovered where the test expects two, meaning a real two-posting email (an ordinal-numbered "Position: 1"/"Position: 2" label paired with a separate "Title:" label per posting) is being collapsed into a single record. Root cause, read directly from `73bb5d7b`'s own diff (§ above): the new guard added to `_split_requirement_sections` — `if not extract_probable_title(value) or len(value) > 100: continue` — silently drops a section boundary whenever the labeled value at that boundary isn't itself title-shaped; `extract_probable_title("2")` returns `None` for a bare ordinal digit, so the second posting's boundary is discarded entirely rather than falling back to that section's own `Title:` line. Because the CI job's script loop runs under `bash -e` and this script fails before several scripts/test modules later in the same list run, **the tests added in `73bb5d7b` through `061aee28` (`test_email_review_regressions.py`, `test_candidate_lookup.py`, `test_relay_spacing.py`, `test_skill_quality.py`, `test_deterministic_taxonomy.py`) never actually executed in CI on any of these five pushes**, despite being wired into `.github/workflows/hermes-regression.yml` — their "new tests" citations above are code-review-verified only, not CI-green-verified. This regression is still open as of `061aee28`; no fix has landed. **HERMES-850's Testing dimension moves 🟢 → 🔴 in the status matrix above pending a fix** (the module's own CI gate is currently red on `main`), and a new P0 item is added to [§8](#8-priority-matrix).
...
| Priority | Item | Why |
|---|---|---|
| **P0** | Fix the open "Hermes regression suite" CI failure on `main` (`73bb5d7b`..`061aee28`) | `scripts/hermes-850-email-parsing-check.py::test_position_ordinal_label_is_never_read_as_the_job_title` has failed identically on 5 consecutive pushes (confirmed via job logs, not a flake) — `73bb5d7b`'s new title-shape guard in `_split_requirement_sections` silently drops an ordinal-labeled section boundary instead of falling back to that section's own `Title:` line, collapsing a real two-posting email into one record. Because the CI script loop runs under `bash -e`, every script/test module ordered after this one — including all five test files added across this same commit range — has never actually run in CI. See the HERMES-850 section above for the full evidence trail. |
