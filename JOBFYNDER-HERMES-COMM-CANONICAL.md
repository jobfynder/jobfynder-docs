...
**2026-09-12 (v1.11):** Scheduled docs-sync run. Thirteen new commits on `jobfynder/hermes` `main` since v1.10 (`5f239f79..061aee2`). PR #11 (`fix/hermes-850-full-reparse-batch-progress`, flagged unmerged in v1.10) landed as `e4071bf`. The rest split into three threads: a durable backfill-job subsystem replacing the one-shot backfill endpoints (`342cd49` plus supporting commits), a real gap closed in the "email parsing never calls an LLM" guarantee (`b5a245c`), and HERMES-400 taxonomy-governance hardening (`b63bdd7`/`061aee2`) that closes a gap this document did not previously know existed. **The most important thing this run found: `main`'s CI has been red for two days.** See the HERMES-850 section for the full trail, and no new commits on `jobfynder-infra` `main` (`jobfynder_infra_last_sha` is unchanged from v1.10 — nothing to reconcile against the COMM side this run).
...
**Update (2026-09-12, v1.11).** `15c0d260` (`Index pending taxonomy candidate matching to unblock live email intake`) is a straightforward performance fix: `_find_loosely_matching_pending_candidate()` (`app/understanding/taxonomy/candidates.py`) was a full table scan over one signal type's entire pending queue on every lookup; replaced with a `WHERE ... = %s ORDER BY id LIMIT 1` query against a new partial expression index (`idx_taxonomy_candidates_pending_loose`), confirmed via `EXPLAIN` in the new `tests/test_candidate_lookup.py` that the index is actually used.

**A real, previously-undocumented gap in the "unknown terms are never auto-approved" safety rule above, found while reviewing these commits and now closed.** `scripts/hermes-900-daily-taxonomy-triage.py` (commit `b314051`, predates this document's `hermes_last_sha` tracking baseline and was never written up here) runs a `classify_batch()` LLM classifier against pending taxonomy candidates and could auto-approve or auto-reject them — for `skill`, `job_title`, and `boilerplate_line` signal types alike — which is a direct conflict with this section's own stated rule that "unknown terms are never auto-approved." `b63bdd7` (`Recover spaced relay companies and prevent taxonomy noise promotion`) closes this for `skill` candidates first: `triage()` now short-circuits to `skipped_reason: "automatic_skill_approval_disabled"` for that signal type, and a new `app/understanding/taxonomy/skill_quality.py::skill_noise_reason()` rejects standalone prose/metadata terms (`"Overall"`, `"Employment Type"`, a truncated `"PEFT (LoRA"`, an email address) from ever being promoted, whether by the daily triage script or by `approve_taxonomy_candidate()`/`add_canonical_skill()` directly. `061aee2` (`Enforce technical-only taxonomy and deterministic enrichment defaults`) finishes the job: `triage()`'s automatic-approval short-circuit is extended to `job_title` and `boilerplate_line` too (`skipped_reason: "automatic_taxonomy_llm_triage_disabled"` — no signal type is auto-triaged by an LLM anymore), `skill_noise_reason()` gains a non-technical-professional-skill filter (rejects "Stakeholder Management," "Teamwork," "Leadership Coaching," etc., while keeping real technical terms like "Communication Protocols" or "Unified Communications"), and `generate_skill_description()`/`classify_job_title_family()` (`app/understanding/taxonomy/descriptions.py`, `title_family_classifier.py`) both gain an `allow_llm: bool = False` parameter — an LLM call for either now requires an explicit opt-in from the caller, not just Litellm being configured. New tests confirm the LLM is never called without that flag (`tests/test_deterministic_taxonomy.py`) and that the technical-only filter accepts real tools while rejecting soft-skill phrasing (`tests/test_skill_quality.py`). Both commits show `conclusion: success` on GitHub Actions (runs [71](https://github.com/jobfynder/hermes/actions/runs/34625250171) and [73](https://github.com/jobfynder/hermes/actions/runs/34626987597) — though note run 73 as a whole is red for the unrelated HERMES-850 regression documented in that module's section; the taxonomy-specific test files these two commits added passed in both runs, per the job log). This does not change HERMES-400's maturity band — the taxonomy foundation itself was never in question — but it is real evidence that the "never auto-approved" rule had a live exception until this run's commits, so this document is recording it rather than treating the original rule's presence as sufficient proof it always held.
...
**Update (2026-09-12, v1.11).**

**PR #11 merged, then superseded the same day.** `e4071bf` (`fix(drafts): make bounded full-reparse batches actually advance through the backlog`) landed on `main`, fixing the bug the PR title describes: `backfill_full_reparse`'s query ordered by `created_at` with no offset, and a row that turned out unchanged was never written to at all — so a bounded `limit=`-ed batch could never advance past the genuinely-unresolvable rows sitting at the front of the queue. Fixed by ordering by `updated_at` and touching that column even on a no-op row. `2d7c24c` (same day) then found this endpoint still couldn't apply changes at any real scale — a 100-row apply batch measured against production didn't finish in 180s, because every row opened its own transaction for the UPDATE and again for `record_field_provenance`'s INSERT — and batched every row's writes into one transaction per call instead.

**Both were superseded within 24 hours by a durable job-queue rewrite.** `342cd499` (`Fix Graph heartbeat starvation and add durable resumable backfills`) replaces the synchronous backfill endpoints with `app/drafts/backfill.py`: new tables `draft_backfill_jobs`/`draft_backfill_items`, a `pg_advisory_xact_lock`-serialized `create_job()`, and `run_batch()` processing bounded slices with a per-draft 120s SIGALRM deadline, run by a new dedicated `hermes-backfill-worker` container (`scripts/hermes-backfill-worker.py`, added to `docker-compose.yml` with its own heartbeat-file healthcheck). New endpoints: `POST /drafts/backfill/jobs`, `GET /drafts/backfill/jobs`, `GET /drafts/backfill/jobs/{job_id}`, `POST /drafts/backfill/jobs/{job_id}/{pause|resume|cancel}`. The old `POST /drafts/backfill/full-reparse` and `POST /drafts/backfill/signature-company-fill` endpoints now reject `dry_run=False` outright (`HTTP 409`, "Use POST /drafts/backfill/jobs for durable background processing") and cap `limit` to 5-20 for the dry-run-only path that remains. `5c0dcea` and `9cd5045` (same day) harden `app/runtime/db.py` for the resulting multi-process deployment (API + Graph consumer + backfill worker all calling `init_schema()`): a `pg_advisory_xact_lock`/`pg_advisory_xact_lock_shared` pair now serializes schema migrations against both each other and any in-flight `process_channel_intake()`/backfill transaction, with a new `transaction()` context manager making all of a single intake's writes (dedupe key plus draft) commit atomically. `eb1ad1b` merges this line with the NER-warmup line below; `73bb5d7` adds a third job `kind`, `review-reparse`, that snapshots only `needs_review` drafts (excluding any already reviewer-corrected) for a targeted re-run of just the drafts a fix is meant to help, without touching the rest of the backlog.

**The same `342cd499` commit also fixes a starvation bug in the Microsoft Graph notification consumer.** This document had not previously recorded that `scripts/hermes-850-graph-notification-consumer.py` exists — a `hermes-graph-consumer` container (`docker-compose.yml`) designed to consume Graph mail-change notifications from `jobfynder-infra`'s COMM-gateway RabbitMQ queue. That script predates this sync's commit range (introduced in `28a56e2`, 2026-08-31, before this document's `hermes_last_sha` tracking baseline was ever set) and was never written up. **Correction to §7 above, softened after review:** Microsoft Graph is not purely "contract, not live" at the *code* level — this consumer, its retry/dead-letter topology, and its wiring into `process_channel_intake()` are real and complete, not a stub. But this document cannot currently say it *runs* live: `RABBITMQ_URL` ships blank in `.env.example`, and the consumer's own `_validate_startup_config()` raises immediately if it's unset — with `restart: unless-stopped`, an unconfigured `RABBITMQ_URL` on INTEL-1 would look identical, from git history alone, to a healthy running consumer (both are just "the code exists"). Whether it is actually configured and consuming on INTEL-1, versus crash-looping since `28a56e2`, has not been live-verified — no SSH credentials configured in this environment. What remains unimplemented per §7 either way is the authenticated fetch call using real OAuth credentials. `342cd499` fixes a real bug in this consumer regardless of its current live status: CPU-bound parsing inside `_handle_notification` was blocking Pika's heartbeat thread in-process, which RabbitMQ could misread as a dead consumer under load ("Graph heartbeat starvation") if and when the consumer is running. Fixed by moving `_handle_notification` into a one-worker `ProcessPoolExecutor` with a 120s deadline, tuning Pika's own heartbeat/blocked-connection-timeout, and touching a heartbeat file every poll loop that the container's healthcheck reads.

**A real production incident, unrelated to the above: `8917a37` (`fix(startup): load the NER model at app startup, not on the first request that needs it`).** After a container restart, whichever request first hit `parse_email_signature`'s NER fallback path paid the full, `lru_cache`-hidden cost of loading `en_core_web_sm` synchronously — measured at "well over ten minutes" cold on this deployment's hardware, indistinguishable from a hang (confirmed via the review UI serving fine throughout, and `pg_stat_activity` showing nothing running). Adds `warm_ner_pipeline()`, called from a new FastAPI startup event in `app/main.py` alongside the existing schema-init one, moving that cost to container start.

**`73bb5d7` also fixes real title-extraction bugs, backed by a repo-internal writeup (`docs/email-review-repair.md`, `jobfynder/hermes`, not this canonical doc):** `extract_probable_title` (`app/understanding/parsers/basic.py`) previously failed on titles starting with `.NET` (a leading `.` fell outside the match's character class) and on "Urgent need for ..." / "... for X" hiring prefixes; `_split_requirement_sections` (`app/email_parsing/parsers.py`) could split one real posting into false-positive extra sections at a repeated "Role:" recruiter-signature line or a numbered screening-steps list after the signoff. A new `_extract_heading_title()` recovers a title sitting on its own line under a "Job Title"/"Position" heading (flattened-table email shapes). `extract_work_authorization` (`app/understanding/parsers/contact.py`) gained an explicit "Any Visa" pattern. 60 new test cases in `tests/test_email_review_regressions.py`, all real-shaped fixtures.

**`3e57878` adds server-side pagination to the review dashboard and a conservative review-status auto-repair, backed by a repo-internal writeup (`docs/dashboard-review-operations.md`):** new `GET /drafts/page` (`app/drafts/listing.py`) does filtering/search/counts/pagination in PostgreSQL instead of shipping the whole drafts table to the browser — the doc records a same-day production read-only measurement: the old list endpoint returned 15,117 records (~20.35 MB) in 2.69s versus the new first page's 50 records (~27 KB) in 0.34s. `POST /drafts/review/reconcile-ready` (`app/drafts/review_rules.py`, `drafts:publish` permission, `dry_run=True` default) flips a `needs_review` draft back to `draft` only when the parser's own confidence/warnings/requires_review signals are all already clean and no human correction is recorded — it does not publish anything and does not fabricate a missing field.

**`b5a245c` closes a real gap in this module's core "no LLM in the parsing path" claim (§2.1 above).** `process_channel_intake` (`app/channels/service.py`) was calling `understand_document()` for email intake without passing `skip_llm_fallback` — meaning a low-confidence deterministic parse on the general email-channel path *could* fall through to a real LiteLLM call, contradicting `PARSER_METADATA["uses_llm"] = False`. `parse_requirement_email` (`app/email_parsing/parsers.py`) had the same gap on its own separate `understand_document()` call. Both now pass `skip_llm_fallback=True` (unconditionally for the requirement parser; keyed to `request.channel == "email"` for the general intake path). New test `tests/test_email_fallback_boundary.py` asserts this via a call-args check on the wrapped function, not just a mock return value.

**Open regression: `main`'s CI has been red since 2026-09-10, unresolved through the current HEAD.** GitHub Actions ("Hermes regression suite") shows `3e57878` (run [68](https://github.com/jobfynder/hermes/actions/runs/34468669269)) green, then every subsequent push to `main` red: `73bb5d7` → run [69](https://github.com/jobfynder/hermes/actions/runs/34520846850) `failure`, `15c0d260` → run [70](https://github.com/jobfynder/hermes/actions/runs/34529981733) `failure`, `b63bdd7` → run [71](https://github.com/jobfynder/hermes/actions/runs/34625250171) `failure`, `b5a245c` → run [72](https://github.com/jobfynder/hermes/actions/runs/34625845154) `failure`, `061aee2` (current `main` HEAD as of this sync) → run [73](https://github.com/jobfynder/hermes/actions/runs/34626987597) `failure`. The failure is the same assertion in `scripts/hermes-850-email-parsing-check.py::test_position_ordinal_label_is_never_read_as_the_job_title` in every one of the five runs (confirmed via the job log for run 73): a two-posting vendor email using `Position: 1` / `Title: <real title>` / ... / `Position: 2` / `Title: <real title>` now parses as **one** record instead of two (`titles == ['Java Developer with NoSQL and State Client Exp']` instead of the expected two-element list). Root cause, read directly from `73bb5d7`'s own diff to `_split_requirement_sections`: the new guard added in that commit (`if not extract_probable_title(value) or len(value) > 100: continue`, meant to stop a repeated "Role:"/"Position:" recruiter-signature or work-mode line from creating a false section boundary) evaluates the captured value at the `Position:` label itself — which for this real format is the ordinal digit (`"1"`, `"2"`), not a title — so `extract_probable_title("1")` returns `None` and the guard discards *both* legitimate section boundaries, collapsing the two postings into one and silently dropping the second job. None of the four commits pushed after `73bb5d7` fixed this; there is no open PR against `jobfynder/hermes` as of this run. **This document is not treating HERMES-850 as still cleanly Testing: 🟢** — see the status-matrix row above — until this is fixed and a green run is confirmed. Live re-verification on INTEL-1 was not attempted for any of this run's findings: no SSH credentials configured in this environment.

## HERMES-900 — Role Intelligence Packages

**Evidence.** No `HERMES-900*.md` file exists in the repo, and no Role Intelligence Package implementation exists. The architecture (role-specific capability/permission/prompt/workflow packages sitting on top of the shared HERMES-700 orchestration layer, not standalone agents) is written down in the v3.0 index only.

**Naming collision, found 2026-09-12, same pattern as §2.1's "COMM-1."** `scripts/hermes-900-daily-taxonomy-triage.py` exists and does real work (a daily automated LLM-triage pass over pending HERMES-400 taxonomy candidates — see the HERMES-400 v1.11 update above), but it is not Role Intelligence Package work; the "900" in its filename is an unrelated internal numbering choice, not this document's HERMES-900. Flagging this now, before any real Role Package work starts and collides with it for real.
...
| **P0** | Fix the `main`-branch CI regression in `test_position_ordinal_label_is_never_read_as_the_job_title` | Red since `73bb5d7` (2026-09-10), still red at `061aee2` (current HEAD, 2026-09-11) across five consecutive pushes, no open PR addressing it. A real vendor two-posting email format (`Position: <ordinal>` / `Title: <real title>`, repeated) now collapses to one record, silently dropping the second job. Root cause identified in the HERMES-850 section above: `73bb5d7`'s own `_split_requirement_sections` guard rejects the `Position:` line's captured ordinal value as not title-shaped and discards both real section boundaries. |
