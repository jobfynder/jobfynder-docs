...
**The same `342cd499` commit also fixes a starvation bug in the Microsoft Graph notification consumer.** This document had not previously recorded that `scripts/hermes-850-graph-notification-consumer.py` exists and already runs as its own container (`hermes-graph-consumer` in `docker-compose.yml`) consuming Graph mail-change notifications from `jobfynder-infra`'s COMM-gateway RabbitMQ queue — that script predates this sync's commit range (introduced in `28a56e2`, 2026-08-31, before this document's `hermes_last_sha` tracking baseline was ever set) and was never written up. **Correction to §7 above:** Microsoft Graph is not purely "contract, not live" — a live notification-to-intake consumer already exists and runs continuously; what remains genuinely unimplemented per §7 is only the authenticated fetch call using real OAuth credentials. `342cd499` fixes a real production bug in that already-running consumer: CPU-bound parsing inside `_handle_notification` was blocking Pika's heartbeat thread in-process, which RabbitMQ could misread as a dead consumer under load ("Graph heartbeat starvation"). Fixed by moving `_handle_notification` into a one-worker `ProcessPoolExecutor` with a 120s deadline, tuning Pika's own heartbeat/blocked-connection-timeout, and touching a heartbeat file every poll loop that the container's healthcheck reads.
