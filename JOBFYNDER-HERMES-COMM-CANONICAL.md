...
| COMM-900 | Reliability & Governance | 🟨 Integration in Progress | `comm/COMM-300-900-1000-infrastructure-posture.md`. **Two hardening passes, 2026-08-21:** (1) app-wide per-IP rate limiting, live-tested. (2) host firewall (`ufw`) enabled with an explicit allowlist, staged and verified carefully so SSH access wasn't lost; a real TLS-bypass (comm-gateway's port published directly, discovered mid-pass) closed; a weak/default RabbitMQ credential (a literal `CHANGE_THIS_STRONG_PASSWORD` placeholder, committed to git) rotated. Still open: Redis provisioned but unused, `fail2ban` still HTTP-blind (SSH-only jail), two admin ports (81, 9443) still open to the whole internet (not restricted blind, without knowing the admin's real source IP), no WAF, no alerting on the scanner traffic already observed. |
