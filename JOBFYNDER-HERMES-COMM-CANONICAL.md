...
**2026-09-09 (v1.11):** Scheduled docs-sync run. Three new commits on `jobfynder/hermes` `main` since v1.10 (`5f239f79..8917a375`), all HERMES-850: [PR #11](https://github.com/jobfynder/hermes/pull/11) (`fix/hermes-850-full-reparse-batch-progress`, flagged as open and undocumented in v1.10) is merged, landing as `e4071bf` — orders `backfill_full_reparse`'s query by `updated_at` instead of `created_at`, and bumps `updated_at` on a no-op row too, so repeated bounded `limit=`-ed calls actually advance through the 15,000+-row backlog instead of re-fetching the same permanently-stuck oldest rows forever; `2d7c24c` is a follow-on perf fix found while about to run that endpoint against production — a 100-row apply batch didn't finish in 180s because every row opened its own DB transaction, so the write path is now batched into a single transaction per call (`record_field_provenance` gained an optional `cur` parameter to support this, existing call sites unaffected); `8917a37` fixes a separate live incident — a cold `en_core_web_sm` spaCy load (over ten minutes on this deployment's hardware) blocking the first request that hit the NER fallback in `parse_email_signature`, indistinguishable from a hang — by adding `warm_ner_pipeline()` and calling it from a new FastAPI startup event in `app/main.py`, moving the cost to container start. All three commits confirmed green this run via the GitHub Actions API (`.github/workflows/hermes-regression.yml`, "Hermes regression suite"): `e4071bf` → run [61](https://github.com/jobfynder/hermes/actions/runs/34281975255) `conclusion: success`; `2d7c24c` → run [63](https://github.com/jobfynder/hermes/actions/runs/34283401202) `conclusion: success`; `8917a37` → run [65](https://github.com/jobfynder/hermes/actions/runs/34287283164) `conclusion: success`. No new commits on `jobfynder-infra` `main` since v1.10, and the previously-tracked deployed branch `feature/comm-telegram-message-chunking` is unchanged, still `0c33580`. Live verification skipped: no SSH credentials configured in this environment — all three fixes are code- and CI-verified only, not live-reconfirmed on INTEL-1. This does not change HERMES-850's maturity band or any dimension grade — these are reliability/operability fixes to already-🟡-graded Deploy/Integration surface, not new capability.
...
**Update (2026-09-09, v1.11).** [PR #11](https://github.com/jobfynder/hermes/pull/11), flagged above as open, is merged — squash-merged onto `main` as `e4071bf` (base `5f239f797`, the same commit v1.10 recorded as current). Per the PR's own account: `backfill_full_reparse` was measured at ~0.5s/draft against the real backlog (15,000+ `draft_job_requirement` rows), so a single unlimited call would block for roughly two hours in one HTTP request — the intended usage is several bounded `limit=`-ed calls instead. But the underlying query ordered by `created_at` with no offset, and a row that came back unchanged (which includes every genuinely-unresolvable draft, not just buggy ones) was never written to at all — so the same oldest-N rows would come back on every subsequent call with the same `limit`, forever, and a bounded batch could never advance past them. `e4071bf` fixes this by ordering by `updated_at` instead, and — `dry_run=False` only — always bumping `updated_at` on a no-op row too (a lightweight `UPDATE ... SET updated_at = now()`, no payload rewrite), so every row a bounded batch examines moves to the back of the queue whether it needed a fix or not. New test in `scripts/hermes-850-signature-company-fill-check.py` verifies a stuck (unchanged) draft's `updated_at` moves while its payload stays untouched.

`2d7c24c` (`perf(drafts): batch full-reparse writes into one transaction instead of one per row`) is a follow-on perf fix found while about to run the now-fixed endpoint against production: re-parsing itself is CPU-bound (~0.5s/draft, no DB access), but a 100-row apply batch didn't finish in 180s, because every row opened its own `with cursor()` for the `UPDATE` and a separate one for `record_field_provenance`'s `INSERT` — two DB round-trips per row on top of the parsing cost. Splits the function into two phases: phase 1 does all CPU-bound re-parsing with zero DB access, collecting what needs writing; phase 2 (only when `dry_run=False` and there's something to write) opens one transaction and does every `UPDATE`, every no-op `updated_at` touch, and every provenance insert against it, committing once. `record_field_provenance` (`app/email_parsing/provenance.py`) gained an optional `cur` parameter to reuse an already-open cursor — every pre-existing call site is unaffected, since omitting it keeps the prior open-one/commit/close behavior.

`8917a37` (`fix(startup): load the NER model at app startup, not on the first request that needs it`) fixes a separate, real production incident, live-debugged: after a container restart, a request landing on `parse_email_signature`'s `is_pure_forward` → NER fallback path (`_get_ner_pipeline` in `app/email_parsing/signature.py`, `lru_cache`'d per process) paid the full cost of loading `en_core_web_sm` synchronously — measured at well over ten minutes cold on this deployment's hardware, indistinguishable from a hang (other endpoints kept serving normally throughout; `pg_stat_activity` showed nothing running, ruling out a DB lock). Adds `warm_ner_pipeline()` and calls it from a new FastAPI startup event in `app/main.py`, alongside the existing schema-init one, moving the cost to container start. Safe in CI: if `spacy`/the model isn't installed, `_get_ner_pipeline` already returned `None` without raising, so the startup call is a no-op there — a new test (`test_warm_ner_pipeline_never_raises`, `scripts/hermes-850-email-signature-check.py`) confirms this.

All three commits are confirmed green this run via the GitHub Actions API (`.github/workflows/hermes-regression.yml`, "Hermes regression suite"): `e4071bf` → run [61](https://github.com/jobfynder/hermes/actions/runs/34281975255) `conclusion: success`; `2d7c24c` → run [63](https://github.com/jobfynder/hermes/actions/runs/34283401202) `conclusion: success`; `8917a37` → run [65](https://github.com/jobfynder/hermes/actions/runs/34287283164) `conclusion: success`. Live re-verification on INTEL-1 was not attempted: no SSH credentials configured in this environment. None of these three commits have been added to `hermes/HERMES-850-email-parsing-foundation.md` or `hermes-api-route-inventory.md`, which remain frozen as of the module's 2026-08-21 closure — this paragraph is the current record for these changes until/unless those files are revised. This does not change HERMES-850's maturity band or any dimension grade — Testing was already 🟢, and live provider wiring (the reason Deploy/Integration stay 🟡) is unaffected.
