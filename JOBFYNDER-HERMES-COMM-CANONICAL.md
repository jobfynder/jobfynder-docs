...
**Resume field extraction expanded 2026-09-08 (commit `8319c43`, 2026-09-07).** New `app/understanding/parsers/resume_sections.py` deterministically extracts name, location, summary, structured `experience[]` (company/title/dates/location/description), structured `education[]` (institution/degree/field/year), and certifications from a resume — none of these were extracted before. `contact.py`'s phone extraction, previously US-only, now also matches international numbers via the `phonenumbers` library. Fixes a real, named bug: "Jake-style" resumes were returning empty name/phone/experience/education/certs. Covered by two new test files (`tests/understanding/test_resume_sections.py`, `tests/understanding/test_basic_resume_parse.py`) asserting against a real fixture resume. See `hermes/HERMES-200-understanding-foundation.md` for the full field list.

**Gap.** File itself lists open "Next" items: bigger taxonomy, more JD fields (resume fields substantially addressed by the above), real Unstructured.io integration, Langfuse/Promptfoo/Great Expectations evaluation — none of the remaining items confirmed done as of this document.
