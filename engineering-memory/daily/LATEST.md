# Engineering Memory

Date: 2026-08-24

Status: green

---

# Executive Summary

GitHub push event processed for jobfynder/hermes on branch main.

---

# Repositories

- jobfynder/hermes

---

# Completed Today

- Source: GitHub webhook
- Repository: jobfynder/hermes
- Branch: main
- Head SHA: 77f1f05
- Commit count: 140
- Triggered by: jobfynder
- Commits:
- 5fe8620 feat: add mission control API — Jobfynder Automation
- 52a2630 feat: add persistent mission board — Jobfynder Automation
- 19f6720 feat: add session brief API — Jobfynder Automation
- dfcd0ca refactor: use workspace as memory source of truth — Jobfynder Automation
- e245601 feat: add action engine foundation — Jobfynder Automation
- bfbb82c feat: add action engine REST API — Jobfynder Automation
- 17d93dd feat: add decision engine foundation — Jobfynder Automation
- 07f0219 feat: convert decisions into actions — Jobfynder Automation
- cea8f3e feat: add baseline decision detection — Jobfynder Automation
- 2858030 feat: persist detected decisions as actions — Jobfynder Automation
- 554f8db feat: add workspace API endpoint — Jobfynder Automation
- 74a3982 feat(core): add Hermes config and health metadata — Jobfynder Automation
- c8a1c91 feat(core): add RBAC foundation — Jobfynder Automation
- 2ccb9d2 feat(core): wire RBAC permissions into platform routes — Jobfynder Automation
- 4ce78bd chore(core): enable RBAC enforcement — Jobfynder Automation
- 92ecbcc chore(core): use central Hermes version in parser routes — Jobfynder Automation
- 536b5d1 feat(core): protect parser endpoints with RBAC — Jobfynder Automation
- f6c6e59 feat(core): add Hermes RBAC user management script — Jobfynder Automation
- 6791453 chore(core): add Docker build ignore rules — Jobfynder Automation
- b7cb410 test(core): add Hermes smoke test script — Jobfynder Automation
- d0e7e2b docs(hermes-200): define understanding tool stack — Jobfynder Automation
- 8b8b906 feat(hermes-200): add parser foundation and parse-text endpoint — Jobfynder Automation
- 12c11d6 feat(hermes-200): add local document extractors — Jobfynder Automation
- 0835fcd feat(hermes-200): add extraction quality scoring — Jobfynder Automation
- 59d3409 feat(hermes-200): add token budget compression — Jobfynder Automation
- e3fd29f feat(hermes-200): add compressed llm context to understanding result — Jobfynder Automation
- db08581 feat(hermes-200): add parse-file upload endpoint — Jobfynder Automation
- ba71eee feat(hermes-200): prefer markitdown before local extractor fallbacks — Jobfynder Automation
- b6331e5 feat(hermes-200): add basic local skills parser — Jobfynder Automation
- c20498a test(hermes-200): add understanding smoke test script — Jobfynder Automation
- a34043d docs(hermes-200): add understanding progress checkpoint — Jobfynder Automation
- f2c19d8 feat(hermes-200): add structured parsing schemas — Jobfynder Automation
- a80165a feat(hermes-200): wire structured schemas into basic parser — Jobfynder Automation
- e10df79 docs(hermes-200): update understanding progress after schema wiring — Jobfynder Automation
- 7e8020a feat(hermes-200): move skills into editable taxonomy — Jobfynder Automation
- 80ea559 feat(hermes-200): add parser fallback policy engine — Jobfynder Automation
- f525f50 feat(hermes-200): add configurable fallback flags — Jobfynder Automation
- 12b8d4d docs(hermes-200): document fallback config flags — Jobfynder Automation
- bbd55a4 docs(hermes-200): update progress after taxonomy and fallback policy — Jobfynder Automation
- 8dcf427 feat(hermes-200): add skills taxonomy debug endpoint — Jobfynder Automation
- 99f0e39 test(hermes-200): include taxonomy endpoint in smoke test — Jobfynder Automation
- f1afd7e docs(hermes-200): record taxonomy endpoint milestone — Jobfynder Automation
- 220361f feat(hermes-200): add resume contact field extraction — Jobfynder Automation
- 165b89a test(hermes-200): include resume contact extraction in smoke test — Jobfynder Automation
- 71b4a20 docs(hermes-200): record resume contact extraction milestone — Jobfynder Automation
- 58a0745 feat(hermes-200): add job description field extraction — Jobfynder Automation
- a0d7822 test(hermes-200): include job description field extraction in smoke test — Jobfynder Automation
- 0e80942 docs(hermes-200): record job description extraction milestone — Jobfynder Automation
- 68b4ccb feat(hermes-200): add parser output validation — Jobfynder Automation
- 2eb8bc7 test(hermes-200): include parser validation in smoke test — Jobfynder Automation
- a6a873c docs(hermes-200): record parser validation milestone — Jobfynder Automation
- fe73acc feat(hermes-200): add safe unstructured fallback placeholder — Jobfynder Automation
- 8848ddb docs(hermes-200): record unstructured fallback placeholder milestone — Jobfynder Automation
- 7242873 feat(hermes-200): extract required and preferred jd skills — Jobfynder Automation
- 7f865f8 test(hermes-200): include required preferred jd skills in smoke test — Jobfynder Automation
- 73874a0 docs(hermes-200): record required preferred jd skills milestone — Jobfynder Automation
- 2b58a20 feat(hermes-200): add parser result version metadata — Jobfynder Automation
- a4e5702 test(hermes-200): include parser version metadata in smoke test — Jobfynder Automation
- 5d9ea95 docs(hermes-200): record parser version metadata milestone — Jobfynder Automation
- 27dd9aa feat(hermes-200): add document quality thresholds — Jobfynder Automation
- 104f868 test(hermes-200): include quality thresholds in smoke test — Jobfynder Automation
- 5a5c7aa docs(hermes-200): record document quality threshold milestone — Jobfynder Automation
- 50194ca docs(hermes-200): add consolidated understanding checkpoint — Jobfynder Automation
- ff4cd4b feat(hermes-300): add matching data models — Jobfynder Automation
- 2b99ebc feat(hermes-300): add basic resume to job scorer — Jobfynder Automation
- fe4532d feat(hermes-300): expose resume to job matching endpoint — Jobfynder Automation
- 7022cb6 test(hermes-300): add matching endpoint smoke coverage — Jobfynder Automation
- c318567 test(hermes-300): add matching decision scenario coverage — Jobfynder Automation
- 7d3c399 feat(hermes-300): add understanding to matching adapter — Jobfynder Automation
- ad685d6 test(hermes-300): add understanding adapter scenario coverage — Jobfynder Automation
- 7a44cfc feat(hermes-300): add matching endpoint for understanding output — Jobfynder Automation
- 4032baf test(hermes-300): add understanding output endpoint smoke coverage — Jobfynder Automation
- 10da47f fix(hermes-300): improve matching explanation formatting — Jobfynder Automation
- 54da152 refactor(hermes-300): extract matching scoring policy — Jobfynder Automation
- 2ed2fbc test(hermes-300): add matching policy validation guardrail — Jobfynder Automation
- bde627d feat(hermes-300): expose active matching policy endpoint — Jobfynder Automation
- 2c5968f feat(hermes-300): include policy snapshot in match results — Jobfynder Automation
- 08caf87 feat(hermes-400): add taxonomy foundation — Jobfynder Automation
- ed08769 feat(hermes-400): expose taxonomy endpoints — Jobfynder Automation
- 7bd62c0 feat(hermes-400): add taxonomy signal extraction — Jobfynder Automation
- 936570c chore(hermes-400): clean taxonomy model whitespace — Jobfynder Automation
- 55a35a0 feat(hermes-400): expose taxonomy signal extraction endpoint — Jobfynder Automation
- ebb1e82 feat(hermes-400): add taxonomy signals to understanding output — Jobfynder Automation
- ed9a8bb feat(hermes-400): use taxonomy-normalized skills in matching adapter — Jobfynder Automation
- 740fc05 feat(hermes-400): add taxonomy suggestion queue foundation — Jobfynder Automation
- 71dc08d feat(hermes-400): expose taxonomy suggestion endpoint — Jobfynder Automation
- b17b8ac feat(hermes-400): add taxonomy snapshot foundation — Jobfynder Automation
- 1f21f8a feat(hermes-400): expose taxonomy snapshot endpoint — Jobfynder Automation
- 9625129 feat(hermes-500): add submission intelligence foundation — Jobfynder Automation
- 4c7c962 feat(hermes-500): add submission handoff adapters — Jobfynder Automation
- bfe7053 feat(hermes-500): add deterministic workflow rules — Jobfynder Automation
- 2d8c1ab docs(hermes-500): add submission intelligence API fixtures — Jobfynder Automation
- a15a665 feat(hermes-600): add integrations core foundation — Jobfynder Automation
- ab2fb2e feat(hermes-600): expose integrations API router — Jobfynder Automation
- 90e9567 test(hermes-600): make integrations API check docker-aware — Jobfynder Automation
- 7d26acf feat(hermes-600): add Jobfynder submission handoff adapter — Jobfynder Automation
- 7ba2584 feat(hermes-600): expose Jobfynder submission handoff API — Jobfynder Automation
- 57c825a docs(hermes-600): add integrations API fixtures — Jobfynder Automation
- 26afc9b feat(hermes-600): add integration retry policy — Jobfynder Automation
- 305c031 feat(hermes-600): expose integration retry policy API — Jobfynder Automation
- 78f83b6 feat(hermes-600): add integration event identity — Jobfynder Automation
- f945748 feat(hermes-600): expose integration event identity API — Jobfynder Automation
- 7c23d1f docs(hermes-600): add retry and identity API fixtures — Jobfynder Automation
- 37ce96a feat(hermes-700): add agent registry and dry-run router — Jobfynder Automation
- 42eccb9 test(hermes-700): add agents API check and fixtures — Jobfynder Automation
- 1ddf34c feat(hermes-700): enforce agent policy decisions — Jobfynder Automation
- b62612f test(hermes-700): refresh policy audit API fixtures — Jobfynder Automation
- 2186580 feat(hermes-700): add agent handoff envelope — Jobfynder Automation
- 6f90507 feat(hermes-700): add agent audit event — Jobfynder Automation
- 301680a test(hermes-700): fix agent audit registry assertions — Jobfynder Automation
- c2fc718 feat(hermes-700): add agents snapshot endpoint — Jobfynder Automation
- c437e4a feat(hermes-450): add channel intake foundation — Jobfynder Automation
- c484f9d feat(hermes-450): add file intake foundation — Jobfynder Automation
- 066e23a feat(hermes-450): align file intake response contract — Jobfynder Automation
- 7cda059 feat(hermes-450): add draft persistence and intake event logs — Jobfynder Automation
- 045a54e feat(hermes-450): add channel adapters and onboarding API — Jobfynder Automation
- 5eba9f7 feat(hermes-450): add role action access gate — Jobfynder Automation
- bc4a4b9 feat(hermes-450): add provider registry telegram workflows and onboarding verification — Jobfynder Automation
- 1caa4eb feat(hermes-450): secure provider contracts and complete channel bridges — Jobfynder Automation
- a972947 test(hermes-450): add provider contract verification script — Jobfynder Automation
- 98b221c feat(hermes-450): add signed internal COMM intake endpoint — Jobfynder Automation
- 134c3b4 merge(hermes): consolidate hermes-450 through hermes-700 runtime base — Jobfynder Automation
- 1869e82 feat(hermes-750): add Portkey-ready prompt runtime foundation — Jobfynder Automation
- ed84e3f feat(hermes-800): add deterministic resume builder foundation — Jobfynder Automation
- 9b1e0a4 feat(hermes-800): add dry-run resume suggestion adapters — Jobfynder Automation
- 7a204a7 feat(hermes-800): add deterministic skill normalization — Jobfynder Automation
- a2d1b49 feat(hermes-800): add deterministic resume tailoring analysis — Jobfynder Automation
- b2592e9 feat(hermes-800): add deterministic resume quality analysis — Jobfynder Automation
- d9196b1 test(hermes-800): add API fixtures and foundation validation — Jobfynder Automation
- 5f08c22 feat(hermes-850): add deterministic email parsing foundation — Jobfynder Automation
- fd2f088 fix(hermes-850): enforce email parser guardrails — Jobfynder Automation
- e28a86e checkpoint: capture uncommitted frozen-v1 state (Context Cards, broadcast extraction, prompt runtime, cache, resume-builder additions) — Jobfynder Automation
- 0dce803 feat(hermes-850): add Gmail and Microsoft Graph email provider connectors — Jobfynder Automation
- 2154e93 feat(hermes-850): implement the fetch-from-provider step for Gmail/Graph — Pavan
- 4b7d0a5 feat(drafts): add reject_draft_object, mirroring publish — Pavan
- 8a5c904 feat(hermes-850): Graph subscription lifecycle + clientState verification — Pavan
- fc81032 feat(hermes-850): confidence-based hotlist/requirement classification for a shared mailbox — Pavan
- 786c99c feat(hermes-850): resolve original sender for forwarded email (spec 4.1) — Jobfynder Automation
- 0116b78 feat(hermes-850): exact-content dedupe, field provenance, claim-and-verify loop — Jobfynder Automation
- 77f1f05 merge: bring in trivial main-side commit before pushing hermes-850 work to main — Jobfynder Automation
- Changed files:
- added: .dockerignore
- added: app/access/__init__.py
- added: app/access/models.py
- added: app/access/registry.py
- added: app/access/service.py
- added: app/action_engine/__init__.py
- added: app/action_engine/models.py
- added: app/action_engine/service.py
- added: app/agents/__init__.py
- added: app/agents/models.py
- added: app/agents/service.py
- added: app/channels/__init__.py
- added: app/channels/adapters/__init__.py
- added: app/channels/adapters/base.py
- added: app/channels/adapters/contracts.py
- added: app/channels/adapters/generic.py
- added: app/channels/adapters/registry.py
- added: app/channels/adapters/telegram.py
- added: app/channels/broadcast_extraction.py
- added: app/channels/models.py
- added: app/channels/registry.py
- added: app/channels/service.py
- added: app/claim/__init__.py
- added: app/claim/models.py
- added: app/claim/service.py
- added: app/config.py
- added: app/context/__init__.py
- added: app/context/candidate_card.py
- added: app/context/conversation_context.py
- added: app/context/job_card.py
- added: app/context/models.py
- added: app/context/relationship_card.py
- added: app/contracts/__init__.py
- added: app/contracts/envelope.py
- added: app/decision_engine/__init__.py
- added: app/decision_engine/models.py
- added: app/decision_engine/service.py
- added: app/drafts/__init__.py
- added: app/drafts/models.py
- added: app/drafts/service.py
- added: app/email_parsing/__init__.py
- added: app/email_parsing/dedupe.py
- added: app/email_parsing/parsers.py
- added: app/email_parsing/provenance.py
- added: app/email_parsing/routing.py
- added: app/email_parsing/sender_resolver.py
- added: app/intake/__init__.py
- added: app/intake/models.py
- added: app/intake/service.py
- added: app/intake/storage.py
- added: app/integrations/__init__.py
- added: app/integrations/jobfynder.py
- added: app/integrations/models.py
- added: app/integrations/service.py
- added: app/matching/__init__.py
- added: app/matching/adapters.py
- added: app/matching/models.py
- added: app/matching/policy.py
- added: app/matching/scorer.py
- added: app/mission_control/models.py
- added: app/mission_control/service.py
- added: app/onboarding/__init__.py
- added: app/onboarding/models.py
- added: app/onboarding/service.py
- added: app/onboarding/text_parser.py
- added: app/prompt_runtime/__init__.py
- added: app/prompt_runtime/extraction_fallback.py
- added: app/prompt_runtime/langfuse_prompts.py
- added: app/prompt_runtime/models.py
- added: app/prompt_runtime/registry.json
- added: app/prompt_runtime/registry.py
- added: app/prompt_runtime/run_log.py
- added: app/prompt_runtime/safety.py
- added: app/prompt_runtime/service.py
- added: app/providers/__init__.py
- added: app/providers/brightdata/__init__.py
- added: app/providers/brightdata/service.py
- added: app/providers/email/__init__.py
- added: app/providers/email/service.py
- added: app/providers/gmail/__init__.py
- added: app/providers/gmail/service.py
- added: app/providers/google_chat/__init__.py
- added: app/providers/google_chat/service.py
- added: app/providers/linkedin/__init__.py
- added: app/providers/linkedin/service.py
- added: app/providers/microsoft_graph/__init__.py
- added: app/providers/microsoft_graph/service.py
- added: app/providers/models.py
- added: app/providers/registry.py
- added: app/providers/slack/__init__.py
- added: app/providers/slack/service.py
- added: app/providers/teams/__init__.py
- added: app/providers/teams/service.py
- added: app/providers/telegram/__init__.py
- added: app/providers/telegram/messages.py
- added: app/providers/telegram/service.py
- added: app/providers/whatsapp/__init__.py
- added: app/providers/whatsapp/service.py
- added: app/resume_builder/__init__.py
- added: app/resume_builder/adapters.py
- added: app/resume_builder/ats_rules.py
- added: app/resume_builder/feedback.py
- added: app/resume_builder/models.py
- added: app/resume_builder/quality.py
- added: app/resume_builder/safety.py
- added: app/resume_builder/service.py
- added: app/resume_builder/tailoring.py
- added: app/resume_builder/taxonomy.py
- added: app/routers/access.py
- added: app/routers/actions.py
- added: app/routers/agents.py
- added: app/routers/brightdata_provider.py
- added: app/routers/broadcast.py
- added: app/routers/channels.py
- added: app/routers/claim.py
- added: app/routers/comm_intake.py
- added: app/routers/context.py
- added: app/routers/drafts.py
- added: app/routers/email_provider.py
- added: app/routers/gmail_provider.py
- added: app/routers/integrations.py
- added: app/routers/linkedin_provider.py
- added: app/routers/matching.py
- added: app/routers/microsoft_graph_provider.py
- added: app/routers/mission_control.py
- added: app/routers/onboarding.py
- added: app/routers/prompts.py
- added: app/routers/provider_contracts.py
- added: app/routers/providers.py
- added: app/routers/resume_builder.py
- added: app/routers/security.py
- added: app/routers/session_brief.py
- added: app/routers/submissions.py
- added: app/routers/telegram_provider.py
- added: app/routers/understanding.py
- added: app/routers/workspace.py
- added: app/runtime/__init__.py
- added: app/runtime/cache.py
- added: app/runtime/events.py
- added: app/runtime/intake_log.py
- added: app/runtime/jsonl_store.py
- added: app/security/__init__.py
- added: app/security/comm_signature.py
- added: app/security/fastapi_comm.py
- added: app/security/rbac.py
- added: app/services/messaging_actions.py
- added: app/session_brief/models.py
- added: app/session_brief/service.py
- added: app/sessions/__init__.py
- added: app/sessions/models.py
- added: app/sessions/service.py
- added: app/submission_intelligence/__init__.py
- added: app/submission_intelligence/adapters.py
- added: app/submission_intelligence/extraction.py
- added: app/submission_intelligence/lifecycle.py
- added: app/submission_intelligence/models.py
- added: app/submission_intelligence/service.py
- added: app/understanding/__init__.py
- added: app/understanding/compression/__init__.py
- added: app/understanding/compression/token_budget.py
- added: app/understanding/extractors/__init__.py
- added: app/understanding/extractors/docx_extractor.py
- added: app/understanding/extractors/local_file.py
- added: app/understanding/extractors/markitdown_extractor.py
- added: app/understanding/extractors/pdf_extractor.py
- added: app/understanding/extractors/plain_text.py
- added: app/understanding/extractors/unstructured_extractor.py
- added: app/understanding/fallback_policy.py
- added: app/understanding/llm_fallback.py
- added: app/understanding/models.py
- added: app/understanding/parsers/__init__.py
- added: app/understanding/parsers/basic.py
- added: app/understanding/parsers/contact.py
- added: app/understanding/parsers/job_description_fields.py
- added: app/understanding/parsers/skills.py
- added: app/understanding/quality/__init__.py
- added: app/understanding/quality/scoring.py
- added: app/understanding/quality/thresholds.py
- added: app/understanding/service.py
- added: app/understanding/structured.py
- added: app/understanding/taxonomy/__init__.py
- added: app/understanding/taxonomy/canonical_skills.json
- added: app/understanding/taxonomy/job_titles.json
- added: app/understanding/taxonomy/loader.py
- added: app/understanding/taxonomy/models.py
- added: app/understanding/taxonomy/normalizer.py
- added: app/understanding/taxonomy/signals.py
- added: app/understanding/taxonomy/skill_aliases.json
- added: app/understanding/taxonomy/skills.json
- added: app/understanding/taxonomy/suggestions.py
- added: app/understanding/taxonomy/title_aliases.json
- added: app/understanding/taxonomy/versioning.py
- added: app/understanding/validation.py
- added: app/workspace/__init__.py
- added: app/workspace/models.py
- added: app/workspace/service.py
- added: docs/hermes-200/consolidated-checkpoint.md
- added: docs/hermes-200/progress.md
- added: docs/hermes-200/tool-stack-decision.md
- added: docs/hermes-500/api-fixtures/README.md
- added: docs/hermes-500/api-fixtures/evaluate-from-handoff-duplicate-request.json
- added: docs/hermes-500/api-fixtures/evaluate-from-handoff-duplicate-response.json
- added: docs/hermes-500/api-fixtures/evaluate-from-handoff-request.json
- added: docs/hermes-500/api-fixtures/evaluate-from-handoff-response.json
- added: docs/hermes-500/api-fixtures/evaluate-intro-request.json
- added: docs/hermes-500/api-fixtures/evaluate-intro-response.json
- added: docs/hermes-500/api-fixtures/evaluate-invalid-transition-request.json
- added: docs/hermes-500/api-fixtures/evaluate-invalid-transition-response.json
- added: docs/hermes-500/api-fixtures/workflow-policy-response.json
- added: docs/hermes-600/api-fixtures/README.md
- added: docs/hermes-600/api-fixtures/event-identity-request.json
- added: docs/hermes-600/api-fixtures/event-identity-response.json
- added: docs/hermes-600/api-fixtures/event-normalize-needs-review-request.json
- added: docs/hermes-600/api-fixtures/event-normalize-needs-review-response.json
- added: docs/hermes-600/api-fixtures/event-normalize-request.json
- added: docs/hermes-600/api-fixtures/event-normalize-response.json
- added: docs/hermes-600/api-fixtures/integrations-health-response.json
- added: docs/hermes-600/api-fixtures/jobfynder-submission-handoff-duplicate-request.json
- added: docs/hermes-600/api-fixtures/jobfynder-submission-handoff-duplicate-response.json
- added: docs/hermes-600/api-fixtures/jobfynder-submission-handoff-request.json
- added: docs/hermes-600/api-fixtures/jobfynder-submission-handoff-response.json
- added: docs/hermes-600/api-fixtures/retry-decision-request.json
- added: docs/hermes-600/api-fixtures/retry-decision-response.json
- added: docs/hermes-600/api-fixtures/retry-policy-response.json
- added: docs/hermes-700/api-fixtures/agents-health-response.json
- added: docs/hermes-700/api-fixtures/agents-registry-response.json
- added: docs/hermes-700/api-fixtures/agents-snapshot-response.json
- added: docs/hermes-700/api-fixtures/bench-sales-agent-response.json
- added: docs/hermes-700/api-fixtures/blocked-execute-response.json
- added: docs/hermes-700/api-fixtures/safe-dry-run-response.json
- added: docs/hermes-700/api-fixtures/unknown-agent-response.json
- added: docs/hermes-750/api-fixtures/prompt-run-dry-run-request.json
- added: docs/hermes-750/api-fixtures/prompt-runtime-health-response.json
- added: docs/hermes-800/api-fixtures/analyze-request.json
- added: docs/hermes-800/api-fixtures/analyze-response.json
- added: docs/hermes-800/api-fixtures/bullet-suggest-request.json
- added: docs/hermes-800/api-fixtures/quality-analyze-request.json
- added: docs/hermes-800/api-fixtures/resume-builder-health-response.json
- added: docs/hermes-800/api-fixtures/resume-builder-policy-response.json
- added: docs/hermes-800/api-fixtures/skills-normalize-request.json
- added: docs/hermes-800/api-fixtures/summary-suggest-request.json
- added: docs/hermes-800/api-fixtures/tailor-request.json
- added: scripts/hermes-200-smoke-test.sh
- added: scripts/hermes-300-matching-scenarios.py
- added: scripts/hermes-300-policy-validation.py
- added: scripts/hermes-300-understanding-adapter-scenario.py
- added: scripts/hermes-400-matching-taxonomy-adapter-check.py
- added: scripts/hermes-400-signal-extraction-check.py
- added: scripts/hermes-400-suggestion-queue-check.py
- added: scripts/hermes-400-taxonomy-api-check.py
- added: scripts/hermes-400-taxonomy-snapshot-check.py
- added: scripts/hermes-400-taxonomy-validation.py
- added: scripts/hermes-400-understanding-taxonomy-check.py
- added: scripts/hermes-450-channel-intake-check.py
- added: scripts/hermes-450-provider-verification.py
- added: scripts/hermes-500-api-fixture-check.py
- added: scripts/hermes-500-handoff-adapter-check.py
- added: scripts/hermes-500-submission-intelligence-check.py
- added: scripts/hermes-500-workflow-rules-check.py
- added: scripts/hermes-600-api-fixture-check.py
- added: scripts/hermes-600-event-identity-api-check.py
- added: scripts/hermes-600-event-identity-check.py
- added: scripts/hermes-600-integrations-api-check.py
- added: scripts/hermes-600-integrations-core-check.py
- added: scripts/hermes-600-jobfynder-adapter-check.py
- added: scripts/hermes-600-jobfynder-api-check.py
- added: scripts/hermes-600-retry-api-check.py
- added: scripts/hermes-600-retry-policy-check.py
- added: scripts/hermes-700-agent-api-check.py
- added: scripts/hermes-700-agent-registry-check.py
- added: scripts/hermes-750-prompt-runtime-api-check.py
- added: scripts/hermes-750-prompt-runtime-check.py
- added: scripts/hermes-800-foundation-check.py
- added: scripts/hermes-800-resume-builder-api-check.py
- added: scripts/hermes-800-resume-builder-core-check.py
- added: scripts/hermes-800-resume-builder-quality-check.py
- added: scripts/hermes-800-resume-builder-suggestion-check.py
- added: scripts/hermes-800-resume-builder-tailoring-check.py
- added: scripts/hermes-800-resume-builder-taxonomy-check.py
- added: scripts/hermes-850-claim-check.py
- added: scripts/hermes-850-dedupe-check.py
- added: scripts/hermes-850-email-integration-check.py
- added: scripts/hermes-850-email-parsing-check.py
- added: scripts/hermes-850-gmail-graph-provider-check.py
- added: scripts/hermes-850-graph-subscription-renew.py
- added: scripts/hermes-850-live-fetch-check.py
- added: scripts/hermes-850-provenance-check.py
- added: scripts/hermes-850-sender-resolution-check.py
- added: scripts/hermes-access-control.py
- added: scripts/hermes-smoke-test.sh
- modified: .env.example
- modified: app/action_engine/service.py
- modified: app/agents/models.py
- modified: app/agents/service.py
- modified: app/channels/models.py
- modified: app/channels/service.py
- modified: app/config.py
- modified: app/decision_engine/service.py
- modified: app/drafts/service.py
- modified: app/email_parsing/parsers.py
- modified: app/email_parsing/routing.py
- modified: app/email_parsing/sender_resolver.py
- modified: app/intake/models.py
- modified: app/intake/service.py
- modified: app/integrations/models.py
- modified: app/integrations/service.py
- modified: app/main.py
- modified: app/matching/adapters.py
- modified: app/matching/models.py
- modified: app/matching/policy.py
- modified: app/matching/scorer.py
- modified: app/mission_control/service.py
- modified: app/onboarding/models.py
- modified: app/onboarding/service.py
- modified: app/prompt_runtime/models.py
- modified: app/prompt_runtime/registry.json
- modified: app/prompt_runtime/registry.py
- modified: app/prompt_runtime/safety.py
- modified: app/prompt_runtime/service.py
- modified: app/providers/email/service.py
- modified: app/providers/gmail/service.py
- modified: app/providers/microsoft_graph/service.py
- modified: app/providers/registry.py
- modified: app/resume_builder/adapters.py
- modified: app/resume_builder/models.py
- modified: app/resume_builder/service.py
- modified: app/resume_builder/tailoring.py
- modified: app/routers/actions.py
- modified: app/routers/agents.py
- modified: app/routers/channels.py
- modified: app/routers/consultants.py
- modified: app/routers/drafts.py
- modified: app/routers/engineering_memory.py
- modified: app/routers/gmail_provider.py
- modified: app/routers/health.py
- modified: app/routers/integrations.py
- modified: app/routers/jobs.py
- modified: app/routers/matching.py
- modified: app/routers/messages.py
- modified: app/routers/microsoft_graph_provider.py
- modified: app/routers/mission_control.py
- modified: app/routers/onboarding.py
- modified: app/routers/resume_builder.py
- modified: app/routers/security.py
- modified: app/routers/session_brief.py
- modified: app/routers/submissions.py
- modified: app/routers/understanding.py
- modified: app/routers/workspace.py
- modified: app/services/consultant_service.py
- modified: app/services/intent_service.py
- modified: app/session_brief/service.py
- modified: app/submission_intelligence/models.py
- modified: app/submission_intelligence/service.py
- modified: app/understanding/extractors/local_file.py
- modified: app/understanding/models.py
- modified: app/understanding/parsers/basic.py
- modified: app/understanding/parsers/job_description_fields.py
- modified: app/understanding/parsers/skills.py
- modified: app/understanding/service.py
- modified: app/understanding/structured.py
- modified: app/understanding/taxonomy/loader.py
- modified: app/understanding/taxonomy/models.py
- modified: docker-compose.yml
- modified: docs/hermes-200/progress.md
- modified: docs/hermes-200/tool-stack-decision.md
- modified: docs/hermes-600/api-fixtures/README.md
- modified: docs/hermes-700/api-fixtures/blocked-execute-response.json
- modified: docs/hermes-700/api-fixtures/safe-dry-run-response.json
- modified: docs/hermes-700/api-fixtures/unknown-agent-response.json
- modified: docs/hermes-750/api-fixtures/prompt-runtime-health-response.json
- modified: requirements.txt
- modified: scripts/hermes-200-smoke-test.sh
- modified: scripts/hermes-400-taxonomy-api-check.py
- modified: scripts/hermes-450-channel-intake-check.py
- modified: scripts/hermes-600-api-fixture-check.py
- modified: scripts/hermes-600-integrations-api-check.py
- modified: scripts/hermes-700-agent-api-check.py
- modified: scripts/hermes-700-agent-registry-check.py
- modified: scripts/hermes-850-email-integration-check.py
- modified: scripts/hermes-850-email-parsing-check.py
- modified: scripts/hermes-850-live-fetch-check.py
- modified: scripts/hermes-smoke-test.sh

---

# Architecture Decisions

- ADR-EMI-002 - Engineering Memory accepts GitHub webhook input (accepted)
  - Hermes can generate engineering memory from GitHub webhook repository, branch, commit, author, and changed-file context.

---

# Incidents

- None

---

# Lessons Learned

- Repo-aware engineering memory is more useful than generic repository scanning.
- Webhook payloads provide reliable commit, author, branch, and changed-file context.

---

# Open Items

- Improve event archive to store full webhook payload.
- Add failure alerting for memory automation.
- Add deduplication guard for repeated memory commits.

---

# Tomorrow

Use repo-aware engineering memory as the default source for GitHub-triggered automation.

---

Generated by Hermes Engineering Memory Engine
